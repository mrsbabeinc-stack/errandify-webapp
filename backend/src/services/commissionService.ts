 ...
 */
export async function calculatePayable(
  companyId: number,
  taskAmountCents: number
): Promise<number> {
...
export async function logCommission(
  ...
  const result = await db.query(
    `INSERT INTO commission_logs
     ...
     VALUES (?, ?, ?, ?, ?, NOW())`,
    [companyId, taskAmountCents, commissionRate, commissionAmount, tier]
  );
 ...
 */
export async function ensureCommissionTable(): Promise<void> {
  try {
    await db.query(`
