 ...
 */
export async function getConversationMessages(conversationId: number, limit: number = 50): Promise<MessageData[]> {
  try {
    const result = await db.query(
 ...
 */
export async function markAsDelivered(messageIds: number[]): Promise<void> {
  try {
 ...
 */
export async function searchMessages(conversationId: number, query: string): Promise<MessageData[]> {
  try {
    const result = await db.query(
      `SELECT id, conversation_id, sender_id, text, status, created_at, delivered_at, read_at
       FROM chat_messages
