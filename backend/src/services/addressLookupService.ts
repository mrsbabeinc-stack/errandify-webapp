 ...
 */
export async function lookupAddress(postalCode: string): Promise<AddressLookupData | null> {
 ...
 */
async function queryGoogleGeocoding(postalCode: string): Promise<AddressLookupData | null> {
  ...
  try {
    ...
    if (result.address_components) {
      for (const comp of result.address_components) {
        ...
        if (type === 'street_number') addressComponents.block_number = comp.long_name;
        if (type === 'route') addressComponents.street_name = comp.long_name;
        if (type === 'premise') addressComponents.building_name = comp.long_name;
      }
    }
 ...
 */
async function queryMapboxGeocoding(postalCode: string): Promise<AddressLookupData | null> {
 ...
 */
async function cacheAddressData(data: AddressLookupData): Promise<void> {
  try {
    await db.query(
