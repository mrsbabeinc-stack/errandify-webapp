...
class OfflineNotificationService {
  private pendingEmailTimers: Map<number, NodeJS.Timeout> = new Map();
  ...
  async queueOfflineNotification(
    ...
    try {
      ...
      await db.query(
        `INSERT INTO offline_notifications (recipient_id, sender_id, sender_alias, errand_id, errand_title, message_preview, created_at)
         VALUES ($1, $2, $3, $4, $5, $6, NOW())`,
        [recipientId, senderId, senderAlias, errandId, errandTitle, messagePreview]
      );
   ...
   */
  private scheduleOfflineEmailSend(
    recipientId: number,
    recipientEmail: string,
    delayMs: number
  ): void {
   ...
   */
  async sendOfflineEmailBatch(recipientId: number, recipientEmail: string): Promise<void> {
    try {
      // Get user details
      const userResult = await db.query(
      ...
      if (success) {
        ...
        // Mark as sent
        await db.query(
          `DELETE FROM offline_notifications WHERE recipient_id = $1 AND created_at > $2`,
          [recipientId, tenMinutesAgo]
        );
      }
    } catch (error) {
  ...
  async cleanupOldNotifications(): Promise<void> {
    try {
      ...
      const result = await db.query(
        `DELETE FROM offline_notifications WHERE created_at < $1`,
        [oneHourAgo]
      );
