    ...
    if (postalCode && postalCode.length === 6) {
      ...
      try {
        ...
        if (omResponse.data?.results?.[0]) {
          ...
          // Map postal code prefix to area (e.g., "11" → "Pasir Panjang")
          ...
          area = postalCodeAreas[postalPrefix] || addr.BUILDING_NAME?.trim() || 'Singapore';
          console.log(`[Extract] ✅ OneMap API: ${area}, ${fullAddress}`);
        } else {
          // No OneMap results - default safely
          area = 'Singapore';
