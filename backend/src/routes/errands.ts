...
router.post('/:id/complete', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Referral first-job / loyalty bonuses are credited canonically by
    // creditFirstCompletedErrand() at POST /:id/approve-completion, where the
    // errand actually becomes 'completed'. An inline duplicate used to sit here —
    // it was dead (it queried errands.doer_id, which doesn't exist) and, being
    // keyed to this pending_review step, would have double-credited the referrer
    // if revived. Removed in favour of the single canonical path.
