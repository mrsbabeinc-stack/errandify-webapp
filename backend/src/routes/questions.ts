...
router.get('/:errandId', async (req: Request, res: Response) => {
  try {
...
router.post('/', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const existing = await db.query(
      'SELECT id FROM task_questions WHERE errand_id = $1 AND doer_id = $2',
      [errandId, userId]
    );
    ...
    // Insert the question
    const result = await db.query(
      `INSERT INTO task_questions (errand_id, doer_id, question, created_at)
       VALUES ($1, $2, $3, NOW())
...
router.post('/:questionId/reply', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
