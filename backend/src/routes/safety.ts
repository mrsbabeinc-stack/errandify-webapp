...
import {
  screenErrandForSafety,
  getUserSafetyFlags,
  resolveSafetyFlag,
  checkUserBehaviorPattern,
  submitVulnerabilityReport,
} from '../services/safetyScreening.js';
...
router.get('/resources', async (req: Request, res: Response) => {
  try {
    const result = await db.query(
      `SELECT id, title, category, phone, email, url, hours, description, region, languages
...
router.post('/report', async (req: Request, res: Response) => {
  try {
    ...
    const reportId = await submitVulnerabilityReport(
      reportType,
      description,
      relatedErrandId,
      relatedUserId,
 ...
 */
router.post('/pause-account', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
 ...
 */
router.post('/resume-account', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const userId = parseInt(req.userId || '0', 10);
 ...
 */
router.get('/account-status', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const userId = parseInt(req.userId || '0', 10);
 ...
 */
router.get('/flags', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const userId = parseInt(req.userId || '0', 10);
 ...
 */
router.post('/flags/:flagId/resolve', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const userId = parseInt(req.userId || '0', 10);
 ...
 */
router.get('/user/:userId/flags', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
 ...
 */
router.get('/user/:userId/behavior-pattern', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const requesterId = parseInt(req.userId || '0', 10);
    const { userId } = req.params;
