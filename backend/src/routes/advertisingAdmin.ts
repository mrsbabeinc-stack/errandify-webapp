...
router.get('/campaigns/:id', isAdmin, async (req: any, res: Response) => {
  try {
    const { id } = req.params;
...
router.post('/approve', isAdmin, async (req: any, res: Response) => {
  try {
...
router.post('/reject', isAdmin, async (req: any, res: Response) => {
  try {
...
router.post('/campaigns/:id/pause', isAdmin, async (req: any, res: Response) => {
  try {
    const { id } = req.params;
    const campaign = await campaignModel.getById(parseInt(id));
    if (!campaign) {
...
router.post('/campaigns/:id/end', isAdmin, async (req: any, res: Response) => {
  try {
    const { id } = req.params;
    const campaign = await campaignModel.getById(parseInt(id));
    if (!campaign) {
