...
router.post('/check-criminal-records', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    let verificationResult = await db.query(
    ...
    if (!verification) {
      const createResult = await db.query(
        `INSERT INTO user_verifications (user_id, details)
         VALUES ($1, $2)
         RETURNING *`,
        [userId, JSON.stringify({ nric, name, dateOfBirth })]
      );
    ...
    const updateResult = await db.query(
      `UPDATE user_verifications
       ...
       WHERE user_id = $4
       RETURNING *`,
      [status, restrictions, JSON.stringify(checkResult), userId]
    );
    ...
    await db.query(
      `UPDATE users
       SET verification_status = $1,
           ...
           updated_at = NOW()
       WHERE id = $4`,
      [verificationStatus, accountActive, restrictions, userId]
    );
...
function simulateCriminalRecordsCheck(nric: string, name: string) {
...
function determineUserStatus(checkResult: any) {
  ...
  if (checkResult.offenses) {
    ...
    const hasDisqualifying = checkResult.offenses.some((o: string) =>
      disqualifyingOffenses.includes(o)
    );
    ...
    if (hasDisqualifying) {
      ...
      checkResult.convictions?.forEach((conviction: any) => {
...
function getStatusMessage(status: string, restrictions: string[]): string {
  if (status === 'approved') {
