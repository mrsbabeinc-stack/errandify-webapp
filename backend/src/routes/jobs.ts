...
router.post('/:taskId/complete', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Get task with doer info - match the auth check logic from GET endpoint
    const taskResult = await db.query(
      `SELECT e.id, e.title, e.status, e.description, e.formatted_id as errand_id_formatted, e.accepted_bid_id,
              (SELECT doer_id FROM bids WHERE errand_id = e.id AND status IN ('accepted', 'confirmed', 'confirmed_awaiting_start', 'in_progress', 'completed') LIMIT 1) as doer_id
       FROM errands e
       WHERE e.id = $1`,
      [taskId]
    );
