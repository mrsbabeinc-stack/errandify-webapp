...
export async function logAiDecision(entry: AuditLogEntry): Promise<string> {
  try {
    const result = await db.query(
      `INSERT INTO ai_audit_log (user_id, action, ai_model, reason_code, result_summary)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING id`,
      [
        entry.userId,
...
export async function getAuditLog(userId: number, action?: string, limit: number = 100) {
  try {
