export interface ReasonCodeExplanation {
  code: string;
  category: 'match' | 'filter' | 'rank' | 'alert' | 'error';
  human_readable: string;
  confidence_impact: 'high' | 'medium' | 'low';
  user_facing: string;
}

const REASON_CODE_MAP: Record<string, ReasonCodeExplanation> = {
  // Search reasons
...
export function getReasonExplanation(code: string): ReasonCodeExplanation {
  return (
    REASON_CODE_MAP[code] || {
      code,
      category: 'error',
...
export function formatReasonForUser(code: string): string {
  const explanation = getReasonExplanation(code);
  return explanation.user_facing;
}

export function getAllReasonCodes(): Record<string, ReasonCodeExplanation> {
  return REASON_CODE_MAP;
}

export function getReasonsByCategory(category: 'match' | 'filter' | 'rank' | 'alert' | 'error') {
...
export function buildExplainableResponse(
  ...
  result: any;
  explanation: string;
  reason_codes: Array<{ code: string; user_facing: string }>;
} {
