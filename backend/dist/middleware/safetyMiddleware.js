...
export async function getSafetyDeclaration(userId) {
    try {
...
export async function checkVulnerableAccess(userId) {
    try {
        const result = await db.query('SELECT can_access_vulnerable_categories FROM users WHERE id = $1', [userId]);
...
export async function enforceSafetyDeclaration(req, res, next) {
    try {
        ...
        const isProtectedRoute = PROTECTED_ROUTES_PATTERNS.some(pattern => pattern.test(req.path));
        if (!isProtectedRoute) {
            return next();
        }
...
export function isVulnerableCategory(category) {
...
export async function enforceVulnerableAccess(userId, category) {
