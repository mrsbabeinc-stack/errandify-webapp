...
router.get('/:errandId/activity-log', authMiddleware, async (req, res) => {
    try {
        ...
        const errandResult = await db.query(`SELECT e.asker_id, e.id, e.status,
       ...
       WHERE e.id = $1`, [errandId, userId]);
        ...
        const errandDetailsResult = await db.query('SELECT formatted_id FROM errands WHERE id = $1', [errandId]);
        ...
        if (errand.status && errand.status.includes('completed')) {
            const completionLogCheck = await db.query(`SELECT id FROM errand_activity_log WHERE errand_id = $1 AND activity_type = 'completed' LIMIT 1`, [errandId]);
            if (completionLogCheck.rows.length === 0) {
                // Check if there are completion submissions
                const completionCheck = await db.query(`SELECT submitted_by, completion_notes, created_at FROM completion_submissions WHERE errand_id = $1 ORDER BY created_at DESC LIMIT 1`, [errandId]);
                if (completionCheck.rows.length > 0) {
                    ...
                    await db.query(`INSERT INTO errand_activity_log (errand_id, activity_type, actor_id, actor_name, actor_role, created_at)
             VALUES ($1, $2, $3, $4, $5, $6)`, [errandId, 'completed', submission.submitted_by, doerName, 'doer', submission.created_at]);
        ...
        try {
            activitiesResult = await db.query(`SELECT id, activity_type, actor_name, actor_role, details, created_at
         FROM errand_activity_log
         WHERE errand_id = $1
         ORDER BY created_at ASC`, [errandId]);
...
function getActivityDisplayText(type, actorName, details) {
...
router.post('/:errandId/log-viewed-evidence', authMiddleware, async (req, res) => {
    try {
        const { errandId } = req.params;
        const userId = parseInt(req.userId || '0', 10);
        // Verify user is the asker
        const errandResult = await db.query('SELECT asker_id, status FROM errands WHERE id = $1', [errandId]);
        if (errandResult.rows.length === 0) {
            return res.status(404).json({ error: 'Errand not found' });
        }
        const errand = errandResult.rows[0];
        ...
        // Log the activity
        await db.query(`INSERT INTO errand_activity_log (errand_id, activity_type, actor_id, actor_name, actor_role, created_at)
       VALUES ($1, $2, $3, $4, $5, NOW())`, [errandId, 'completion_evidence_viewed', userId, askerName, 'asker']);
