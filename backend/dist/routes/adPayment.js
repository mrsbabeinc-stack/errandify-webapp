...
router.post('/calculate', authMiddleware, async (req, res) => {
    try {
        ...
        const calculation = await adPaymentService.calculateAdPayment(company_id, campaign_id, campaign.title, budgetInCents);
        res.json({
            success: true,
            data: calculation,
...
router.post('/process', authMiddleware, async (req, res) => {
    try {
        ...
        // Process payment (deduct credits and/or charge Stripe)
        const paymentResult = await adPaymentService.processAdPayment(company_id, campaign_id, campaign.title, budgetInCents, undefined, // stripeCustomerId handled via payment intent
 ...
 */
router.get('/status/:company_id', authMiddleware, async (req, res) => {
    try {
...
router.post('/stripe-session', authMiddleware, async (req, res) => {
    try {
        ...
        const sessionData = await adPaymentService.createStripeSession(company_id, campaign_id, campaign.title, stripe_amount_cents, return_url);
        res.json({
            success: true,
            data: sessionData
        });
 ...
 */
router.post('/confirm-approval', authMiddleware, async (req, res) => {
    try {
       ...
       VALUES ($1, $2, $3, 'campaign_approved_confirmed', NOW())`, [company_id, campaign_id, Math.round(campaign.budget * 100)]);
        res.json({
            success: true,
