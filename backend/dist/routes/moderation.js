...
import { moderateContent, flagContent, getFlaggedContent, reviewFlaggedContent, } from '../services/contentModerationService.js';
const router = Router();
...
router.post('/check', authMiddleware, async (req, res) => {
    try {
        ...
        const result = await moderateContent(content, context || 'task_description');
        res.json(result);
    }
    catch (error) {
...
router.get('/flagged', authMiddleware, async (req, res) => {
    try {
        ...
        const flagged = await getFlaggedContent(parseInt(limit) || 50, severity, status);
...
router.post('/review/:id', authMiddleware, async (req, res) => {
    try {
        ...
        const result = await reviewFlaggedContent(parseInt(id), decision, notes);
        if (!result) {
