...
router.get('/profile', authMiddleware, async (req, res) => {
    try {
        ...
        const result = await db.query('SELECT chas_card_color, chas_verified, chas_verified_at FROM users WHERE id = $1', [userId]);
...
router.post('/verify-manual', authMiddleware, async (req, res) => {
    try {
       ...
       WHERE id = $2`, [chasCardColor, userId]);
...
router.post('/verify-api', authMiddleware, async (req, res) => {
    try {
        ...
        if (!userId)
            return res.status(400).json({ error: 'User ID required' });
        if (!nric)
            return res.status(400).json({ error: 'NRIC required for API verification' });
       ...
       SET chas_card_color = $1,
           chas_verified = true,
           chas_verified_at = NOW()
       WHERE id = $2`, [mockChasColor, userId]);
        res.json({
            success: true,
            data: {
...
router.get('/eligibility/:userId', async (req, res) => {
    try {
        ...
        const result = await db.query('SELECT chas_card_color, chas_verified FROM users WHERE id = $1', [userId]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'User not found' });
        }
        const user = result.rows[0];
