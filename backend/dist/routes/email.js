...
import { templateEventConfirmation, templateEventReminder7Days, templateEventReminder24Hours, templateEventReminder1Hour, templateEventReminderDayOf } from '../templates/emailTemplates';
const router = Router();
/**
 * Send event confirmation email
 * POST /api/email/send-event-confirmation
 */
router.post('/send-event-confirmation', async (req, res) => {
    try {
        ...
        const htmlContent = templateEventConfirmation(userName, eventTitle, eventDate, eventTime, eventLocation, eventDescription);
        // Send email
        await sendEmail({
            to: email,
 ...
 */
router.post('/send-event-reminder-7days', async (req, res) => {
    try {
        ...
        const user = req.user;
        const userName = user?.display_name || 'Community Member';
        const htmlContent = templateEventReminder7Days(userName, eventTitle, eventDate, eventTime, eventLocation, eventLink, agenda, preparation);
        await sendEmail({
            to: email,
 ...
 */
router.post('/send-event-reminder-24hours', async (req, res) => {
    try {
        ...
        const user = req.user;
        const userName = user?.display_name || 'Community Member';
        const htmlContent = templateEventReminder24Hours(userName, eventTitle, eventDate, eventTime, eventLocation, eventLink, agenda);
        await sendEmail({
            to: email,
 ...
 */
router.post('/send-event-reminder-1hour', async (req, res) => {
    try {
        ...
        const user = req.user;
        const userName = user?.display_name || 'Community Member';
        const htmlContent = templateEventReminder1Hour(userName, eventTitle, eventTime, eventLocation, eventLink);
        await sendEmail({
            to: email,
 ...
 */
router.post('/send-event-reminder-dayof', async (req, res) => {
    try {
        const { email, eventTitle, eventDate, eventTime, eventLocation, eventLink, agenda } = req.body;
        if (!email || !eventTitle) {
            return res.status(400).json({
        ...
        const user = req.user;
        const userName = user?.display_name || 'Community Member';
        const htmlContent = templateEventReminderDayOf(userName, eventTitle, eventDate, eventTime, eventLocation, eventLink, agenda);
        await sendEmail({
            to: email,
 ...
 */
router.post('/send-no-offers-reminder', async (req, res) => {
    try {
        ...
        const userName = req.user?.display_name || 'Friend';
        const htmlContent = `
 ...
 */
router.post('/send-errand-start-reminder', async (req, res) => {
    try {
