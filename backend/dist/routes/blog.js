...
router.get('/:slug', async (req, res) => {
    try {
        ...
        const result = await db.query(`SELECT * FROM blog_posts WHERE slug = $1 AND is_published = true`, [slug]);
        ...
        const relatedResult = await db.query(`SELECT id, slug, title, subtitle, featured_image_url, published_at, read_time_minutes
       FROM blog_posts
       WHERE category = $1 AND id != $2 AND is_published = true
       ORDER BY published_at DESC
       LIMIT 3`, [post.category, post.id]);
        res.json({
            success: true,
            data: {
                ...post,
...
router.get('/search', async (req, res) => {
    try {
        ...
        const result = await db.query(`SELECT id, slug, title, subtitle, excerpt, featured_image_url, published_at
       FROM blog_posts
...
router.get('/sitemap.xml', async (req, res) => {
    try {
        const result = await db.query(`SELECT slug, published_at, updated_at FROM blog_posts
       WHERE is_published = true
       ORDER BY published_at DESC`);
