...
router.get('/export', authMiddleware, async (req, res) => {
    try {
        ...
        const errandsResult = await db.query(`SELECT e.*, u.display_name as asker_name
       FROM errands e
       ...
       ORDER BY e.created_at DESC`, [userId]);
        // Get all bids
        const bidsResult = await db.query('SELECT * FROM bids WHERE doer_id = $1 ORDER BY created_at DESC', [userId]);
        // Get all ratings received
        const ratingsResult = await db.query(`SELECT r.*, u.display_name as rater_name, e.title as task_title
       FROM ratings r
       ...
       ORDER BY r.created_at DESC`, [userId]);
        // Get all disputes
        const disputesResult = await db.query(`SELECT * FROM disputes
       WHERE filed_by = $1
       ...
       ORDER BY created_at DESC`, [userId]);
        // Get notifications
        const notificationsResult = await db.query('SELECT * FROM notifications WHERE user_id = $1 ORDER BY created_at DESC', [userId]);
        // Get screening declaration
        const screeningResult = await db.query('SELECT * FROM screening_declarations WHERE user_id = $1', [userId]);
    ...
    try {
        ...
        try {
         ...
         WHERE id = $1`, [userId]);
            ...
            await db.query('UPDATE chat_messages SET message = [DELETED] WHERE sender_id = $1', [userId]);
            // Keep disputes and ratings (for platform records) but anonymize
            await db.query(`UPDATE disputes SET filed_by = 0 WHERE filed_by = $1`, [userId]);
            ...
            await db.query('DELETE FROM screening_declarations WHERE user_id = $1', [userId]);
            ...
            await db.query(`INSERT INTO audit_log (user_id, action, details, timestamp)
         VALUES ($1, 'account_deletion', 'User requested account deletion', NOW())`, [userId]);
