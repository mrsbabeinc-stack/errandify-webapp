...
import { upgradeSubscription, cancelSubscription, } from '../services/subscriptionService.js';
...
router.post('/checkout', authMiddleware, async (req, res) => {
    try {
        ...
        const priceId = STRIPE_PRICES[tier][billingType];
        if (!priceId) {
...
router.post('/downgrade', authMiddleware, async (req, res) => {
    try {
       ...
       WHERE company_id = ?`, [tier, companyId]);
        res.json({
            success: true,
...
router.post('/cancel', authMiddleware, async (req, res) => {
    try {
        ...
        try {
            await db.query(`INSERT INTO churn_tracking (company_id, attempt_date, reason)
         VALUES (?, NOW(), ?)`, [companyId, 'user_initiated']);
        }
        catch (e) {
 ...
 */
router.post('/churn-prevention', authMiddleware, async (req, res) => {
    try {
        const companyId = parseInt(req.companyId || '0', 10);
 ...
 */
router.get('/ad-credits/balance', authMiddleware, async (req, res) => {
    try {
        const companyId = parseInt(req.companyId || '0', 10);
 ...
 */
router.get('/ad-credits/history', authMiddleware, async (req, res) => {
    try {
        const companyId = parseInt(req.companyId || '0', 10);
 ...
 */
router.post('/proration', authMiddleware, async (req, res) => {
    try {
 ...
 */
router.get('/admin/subscriptions/analytics', authMiddleware, async (req, res) => {
    try {
        res.json({
            success: true,
            analytics: {
 ...
 */
router.post('/admin/subscriptions/:companyId/change-tier', authMiddleware, async (req, res) => {
    try {
        const { tier } = req.body;
 ...
 */
router.post('/admin/subscriptions/:companyId/cancel', authMiddleware, async (req, res) => {
    try {
        const companyId = parseInt(req.params.companyId, 10);
        // TODO: Add admin role check
        res.json({
