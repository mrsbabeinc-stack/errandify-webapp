...
router.post('/:taskId/start', authMiddleware, async (req, res) => {
    try {
        ...
        const taskResult = await db.query(`SELECT e.*, b.doer_id FROM errands e
       ...
       WHERE e.id = $1`, [taskId]);
        ...
        // Update task status
        const updateResult = await db.query(`UPDATE errands
       SET status = $1, started_at = NOW(), updated_at = NOW()
       WHERE id = $2
       RETURNING id, status, started_at`, ['in_progress', taskId]);
        // Get asker info for notification
        const askerResult = await db.query('SELECT id, display_name FROM users WHERE id = $1', [task.asker_id]);
...
router.post('/:taskId/complete', authMiddleware, async (req, res) => {
    try {
        ...
        const taskResult = await db.query(`SELECT e.id, e.title, e.status, e.description, e.formatted_id as errand_id_formatted, e.accepted_bid_id, b.doer_id
       FROM errands e
       ...
       WHERE e.id = $1`, [taskId]);
            ...
            for (let i = 0; i < photoUrls.length; i++) {
                ...
                try {
                    ...
                    if (photoModeration.status === 'blocked') {
                        return res.status(400).json({
                            error: `❌ Photo ${i + 1} rejected: ${photoModeration.reason}. ${photoModeration.reason?.includes('contact')
        ...
        const submissionResult = await db.query(`SELECT MAX(submission_number) as max_submission FROM completion_submissions WHERE errand_id = $1`, [taskId]);
       ...
       SET status = $1,
           updated_at = NOW()
       WHERE id = $2
       RETURNING id, status, updated_at`, ['completed', taskId]);
        // Create completion submission record
        await db.query(`INSERT INTO completion_submissions (errand_id, submission_number, completion_notes, photo_urls, submitted_by, status)
       VALUES ($1, $2, $3, $4, $5, $6)`, [taskId, currentSubmissionNumber, completionNotes || '', JSON.stringify(photoUrls || []), doerId, 'pending']);
        // Log completion
        ...
        if (photoUrls && photoUrls.length > 0) {
            ...
            for (let i = 0; i < photoUrls.length; i++) {
                ...
                await db.query(`INSERT INTO task_files (errand_id, errand_id_formatted, submission_number, file_url, file_name, uploaded_by)
           VALUES ($1, $2, $3, $4, $5, $6)`, [taskId, errandIdFormatted, currentSubmissionNumber, photoUrl, fileName, doerId]);
            }
        }
        res.status(201).json({
            success: true,
...
router.post('/:taskId/confirm', authMiddleware, async (req, res) => {
    try {
        const { taskId } = req.params;
        const askerId = parseInt(req.userId || '0', 10);
        // Get task and verify asker
        const taskResult = await db.query(`SELECT e.*, b.doer_id, b.amount FROM errands e
       ...
       WHERE id = $2`, ['completed_confirmed', taskId]);
...
router.post('/:taskId/request-more-work', authMiddleware, async (req, res) => {
    try {
        const { taskId } = req.params;
        const askerId = parseInt(req.userId || '0', 10);
       ...
       WHERE id = $2`, ['in_progress', taskId]);
        ...
        const doerResult = await db.query('SELECT id, display_name FROM users WHERE id = $1', [task.doer_id]);
...
router.get('/:taskId/photos', authMiddleware, async (req, res) => {
    try {
        const { taskId } = req.params;
        const userId = parseInt(req.userId || '0', 10);
        // Verify user is involved in task
        const taskResult = await db.query('SELECT asker_id, accepted_bid_id FROM errands WHERE id = $1', [taskId]);
        if (taskResult.rows.length === 0) {
            return res.status(404).json({ error: 'Task not found' });
        }
        // Check authorization
        ...
        if (task.accepted_bid_id) {
            const bidResult = await db.query('SELECT doer_id FROM bids WHERE id = $1', [task.accepted_bid_id]);
        ...
        // Get photos
        const photosResult = await db.query('SELECT id, photo_url, uploaded_by, uploaded_at FROM task_photos WHERE task_id = $1 ORDER BY uploaded_at ASC', [taskId]);
        res.json({
            success: true,
...
async function releasePayment(taskId, task, reason) {
    try {
        ...
        const doerResult = await db.query('SELECT penalty_owed, display_name, alias FROM users WHERE id = $1', [task.doer_id]);
        ...
        try {
            ...
            const doerAccountResult = await db.query('SELECT stripe_account_id FROM users WHERE id = $1', [task.doer_id]);
            if (doerAccountResult.rows[0]?.stripe_account_id) {
                // Create transfer to doer's Stripe Connect account
                const transfer = await stripeService.createTransfer(finalPayout, doerAccountResult.rows[0].stripe_account_id, taskId, reason);
        ...
        const releaseResult = await db.query(`INSERT INTO payment_releases (task_id, bid_amount, platform_fee, doer_payout, stripe_transfer_id, released_at, release_reason)
       VALUES ($1, $2, $3, $4, $5, NOW(), $6)
       RETURNING id, released_at`, [taskId, bidAmount, platformFee, finalPayout, stripeTransferId, reason]);
        // Deduct penalty if applied
        if (penaltyOwed > 0) {
            await db.query('UPDATE users SET penalty_owed = 0 WHERE id = $1', [task.doer_id]);
        }
        ...
        try {
            ...
            await createNotification(task.doer_id, 'payment_released', 'Payment Released', `Payment of SGD $${finalPayout.toFixed(2)} released for errand ${errandId} "${task.title}". Arrives in 1-2 business days.`, null).catch(err => console.warn('[Payment] Failed to notify doer:', err));
            // Notify asker of payment sent
            await createNotification(task.asker_id, 'payment_sent', 'Payment Sent', `Payment of SGD $${finalPayout.toFixed(2)} sent to ${doerDisplay} for errand ${errandId} "${task.title}" (after 20% platform fee).`, null).catch(err => console.warn('[Payment] Failed to notify asker:', err));
        }
        catch (notifErr) {
...
router.get('/:taskId/submissions', authMiddleware, async (req, res) => {
    try {
        ...
        const authCheckResult = await db.query(`SELECT e.asker_id, e.id,
       ...
       WHERE e.id = $1`, [taskId]);
        ...
        const submissionsResult = await db.query(`SELECT
        cs.id,
        cs.errand_id,
       ...
       ORDER BY cs.submission_number ASC`, [taskId]);
        // Get all files organized by submission with Errand ID format
        const filesResult = await db.query(`SELECT
        id,
        errand_id,
        errand_id_formatted,
        submission_number,
       ...
       FROM task_files
       WHERE errand_id = $1
       ORDER BY submission_number DESC, created_at ASC`, [taskId]);
