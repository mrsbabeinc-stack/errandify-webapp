...
router.post('/review-analyzer', authMiddleware, async (req, res) => {
    try {
...
router.post('/rank-doers', authMiddleware, async (req, res) => {
    try {
        ...
        const rankedDoers = await Promise.all(candidate_doers.map(async (doer) => {
            const ratingResult = await db.query(`SELECT AVG(CAST(rating_score AS FLOAT)) as avg_rating, COUNT(*) as count
           FROM errand_assignments WHERE doer_id = $1`, [doer.doer_id]);
        ...
        const fairnessAudit = await biasDetector.auditRankingFairness(rankedDoers, audit_depth);
...
router.post('/suggest-recurrence', authMiddleware, async (req, res) => {
    try {
        ...
        else if (normalizedCategory === 'homehelp' ||
        ...
        else if (normalizedCategory === 'delivery' ||
...
router.get('/audit-log', authMiddleware, async (req, res) => {
    try {
        ...
        const userId = parseInt(req.userId || '0', 10);
        const logs = await privacyLogger.getAuditLog(userId, action, parseInt(limit, 10));
        res.json({
            success: true,
            data: {
                logs,
...
router.get('/bias-audit-summary', authMiddleware, async (req, res) => {
    try {
        const result = await db.query(`SELECT * FROM bias_audit_summary ORDER BY flagged_logs DESC LIMIT 20`);
        res.json({
            success: true,
            data: {
                summary: result.rows,
...
router.post('/extract-task-info', async (req, res) => {
    try {
        let { input } = req.body;
        if (!input)
...
Example: "Decorate Apartment For Party At 238857, Tomorrow 9am" → "Decorate Apartment For Party"`,
        ...
        if (category === 'homehelp' && process.env.QWEN_API_KEY) {
            try {
                ...
                const categoryResponse = await axios.post(`${process.env.QWEN_API_BASE || 'https://dashscope.aliyuncs.com/compatible-mode/v1'}/chat/completions`, {
                    model: 'qwen-max',
                    messages: [
                        {
                            role: 'system',
...
OUTPUT ONLY the category name, nothing else.`,
        ...
        const recurringPatterns = [
            { pattern: /every\s+(\d+)\s+weeks?/i, unit: 'week' },
            { pattern: /every\s+(\d+)\s+days?/i, unit: 'day' },
            { pattern: /every\s+(\d+)\s+months?/i, unit: 'month' },
            { pattern: /weekly/i, unit: 'week', every: 1 },
            { pattern: /daily/i, unit: 'day', every: 1 },
            { pattern: /monthly/i, unit: 'month', every: 1 },
            { pattern: /bi-weekly|biweekly|every\s+other\s+week/i, unit: 'week', every: 2 },
            { pattern: /fortnightly/i, unit: 'week', every: 2 },
        ];
...
router.post('/suggestions', async (req, res) => {
    ...
    try {
        ...
        const [skillsResult, descriptionResult, notesResult] = await Promise.allSettled([
            qwenApiKey ? axios.post(`${process.env.QWEN_API_BASE || 'https://dashscope.aliyuncs.com/compatible-mode/v1'}/chat/completions`.replace('/v1//chat', '/v1/chat'), {
                model: 'qwen-max',
                messages: [
                    {
                        role: 'user',
...
router.post('/analyze-task', authMiddleware, async (req, res) => {
    try {
...
Return ONLY valid JSON, no markdown or code blocks.`;
        try {
            const response = await axios.post(`${process.env.QWEN_API_BASE || 'https://dashscope.aliyuncs.com'}/api/v1/services/aigc/text-generation/generation`, {
                model: 'qwen-max',
                messages: [
                    {
                        role: 'user',
            ...
            try {
                await db.query(`INSERT INTO task_analysis (user_id, title, location_insights, task_complexity, sentiment_analysis, ai_suggestions, created_at)
           VALUES ($1, $2, $3, $4, $5, $6, NOW())
           ON CONFLICT (user_id, title) DO UPDATE SET
           location_insights = $3, task_complexity = $4, sentiment_analysis = $5, ai_suggestions = $6, created_at = NOW()`, [userId, title, JSON.stringify(analysis.location_insights || {}), JSON.stringify(analysis.task_complexity || {}), JSON.stringify(analysis.sentiment_analysis || {}), JSON.stringify(analysis.ai_suggestions || {})]);
            }
            catch (dbErr) {
...
router.post('/analyze-preferences', authMiddleware, async (req, res) => {
    try {
        ...
        const doerServices = doer_preferences?.map((id) => categoryMap[id]).filter(Boolean) || [];
        const askerServices = asker_needs?.map((id) => categoryMap[id]).filter(Boolean) || [];
...
${available_tasks.slice(0, 10).map((t, i) => `${i + 1}. "${t.title}" - ${t.category} (${t.location || 'Location TBD'}, $${t.budget || 'TBD'})`).join('\n')}
...
Return ONLY valid JSON.`;
        try {
            ...
            try {
                await db.query(`INSERT INTO user_suggestions (user_id, suggestions, created_at)
           VALUES ($1, $2, NOW())
           ON CONFLICT (user_id) DO UPDATE SET suggestions = $2, created_at = NOW()`, [userId, JSON.stringify(suggestions)]);
            }
            catch (dbErr) {
