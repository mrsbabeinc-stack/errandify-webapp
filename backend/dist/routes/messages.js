...
import { authMiddleware, isUserOnline } from '../middleware/auth.js';
...
router.post('/tasks/:taskId/send', authMiddleware, async (req, res) => {
    try {
        ...
        const taskResult = await db.query(`SELECT e.*, b.doer_id, u.display_name as asker_name, sender.alias as sender_alias, sender.display_name as sender_name
       FROM errands e
       ...
       WHERE e.id = $1`, [taskId, senderId]);
        ...
        const messageResult = await db.query(`INSERT INTO task_messages (task_id, sender_id, content, flagged, created_at)
       VALUES ($1, $2, $3, $4, NOW())
       RETURNING id, task_id, sender_id, content, flagged, created_at`, [taskId, senderId, content, false]);
        ...
        try {
            ...
            // Get recipient email
            const recipientResult = await db.query(`SELECT email, display_name FROM users WHERE id = $1`, [recipientId]);
          ...
          `,
                ...
                await offlineNotificationService.queueOfflineNotification(recipientId, recipientEmail, senderId, senderAlias, taskId, task.title, content);
            }
        }
        catch (emailErr) {
...
router.get('/tasks/:taskId', authMiddleware, async (req, res) => {
    try {
        ...
        // Verify user is involved in task
        const taskResult = await db.query(`SELECT e.id, e.formatted_id, e.title, e.description, e.category, e.budget, e.deadline,
       ...
       WHERE e.id = $1`, [taskId]);
        ...
        if (task.accepted_bid_id) {
            const acceptedBidResult = await db.query(`SELECT b.doer_id, b.amount, u.display_name, u.alias
         FROM bids b
         LEFT JOIN users u ON b.doer_id = u.id
         WHERE b.id = $1`, [task.accepted_bid_id]);
            ...
            const bidResult = await db.query(`SELECT b.doer_id, b.amount, u.display_name, u.alias
         FROM bids b
         LEFT JOIN users u ON b.doer_id = u.id
         WHERE b.errand_id = $1
        ...
        const messagesResult = await db.query(`SELECT m.id, m.task_id, m.sender_id, m.content, m.flagged, m.created_at,
       ...
       LIMIT 50`, [taskId]);
        ...
        if (task.asker_id || doerId) {
            const statusIds = [task.asker_id, doerId].filter((id) => id != null);
            if (statusIds.length > 0) {
                const placeholders = statusIds.map((_, i) => `$${i + 1}`).join(',');
                const userStatusResult = await db.query(`SELECT id, last_active_at FROM users WHERE id IN (${placeholders})`, statusIds);
                userStatusResult.rows.forEach((row) => {
        ...
        const disputeResult = await db.query(`SELECT id FROM disputes WHERE errand_id = $1 AND status IN ('open', 'in_progress')`, [taskId]);
        ...
        if (!chatDisabled) {
            const completionResult = await db.query(`SELECT updated_at FROM errands WHERE id = $1 AND status = 'completed'`, [taskId]);
        ...
        if (otherUserId) {
            try {
                const favoriteResult = await db.query(`SELECT id FROM user_favorites WHERE user_id = $1 AND favorite_user_id = $2`, [userId, otherUserId]);
...
router.post('/tasks/:taskId/hana', authMiddleware, async (req, res) => {
    try {
        const { taskId } = req.params;
        ...
        // Get task context
        const taskResult = await db.query('SELECT title, description, category, budget FROM errands WHERE id = $1', [taskId]);
        if (taskResult.rows.length === 0) {
            return res.status(404).json({ error: 'Task not found' });
        }
        const task = taskResult.rows[0];
        // Get user language
        const userResult = await db.query('SELECT language_pref FROM users WHERE id = $1', [userId]);
...
async function getRecentFlagCount(userId, taskId) {
    try {
        const result = await db.query(`SELECT COUNT(*) as count FROM task_messages
       WHERE sender_id = $1 AND task_id = $2 AND flagged = true
       AND created_at > NOW() - INTERVAL '24 hours'`, [userId, taskId]);
...
async function getHanaResponse(question, task, userLanguage) {
    try {
        const prompt = `
    ...
    `;
        ...
        return ('Oops, I had a hiccup! Try again in a moment. ' +
            'If this keeps happening, let us know. 🌸');
    }
}
async function callQwen(prompt, language = 'en') {
    try {
        // TODO: Implement real Qwen API call
        // For now, return warm placeholder
        return ('Thanks for reaching out! Our team will help sort this out. ' +
