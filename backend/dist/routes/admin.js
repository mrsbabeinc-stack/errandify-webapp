...
router.post('/admins', isAdmin, async (req, res) => {
    try {
        ...
        const result = await db.query('INSERT INTO admin_users (email, name, role, two_factor_enabled, status, created_at) VALUES (?, ?, ?, ?, ?, NOW())', [email, name, role, twoFactorEnabled ? 1 : 0, 'active']);
