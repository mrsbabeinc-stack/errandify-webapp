...
router.post('/allocate', authMiddleware, async (req, res) => {
    try {
        ...
        const authRes = await db.query(`SELECT role FROM company_staff_assignments
       WHERE company_id = $1 AND user_id = $2 AND status = 'active'`, [company_id, requester_id]);
        ...
        // Get errand details
        const errandRes = await db.query(`SELECT id, title, description, created_by_id
       FROM errands WHERE id = $1 AND created_by_id IN (
        ...
        const staffRes = await db.query(`SELECT user_id, role FROM company_staff_assignments
       WHERE company_id = $1 AND user_id = $2 AND status = 'active'`, [company_id, staff_user_id]);
        ...
        const vulnCheck = await checkVulnerabilityRestrictions(staff_user_id, errand.title, errand.description);
        ...
        const staffDetailRes = await db.query(`SELECT display_name, vulnerability_type FROM users WHERE id = $1`, [staff_user_id]);
        const staffDetail = staffDetailRes.rows[0];
        // Create allocation record
        const allocationRes = await db.query(`INSERT INTO errand_staff_allocations (errand_id, staff_user_id, company_id, allocated_by_user_id, status)
       VALUES ($1, $2, $3, $4, 'allocated')
       ON CONFLICT (errand_id, staff_user_id) DO UPDATE SET status = 'allocated'
       RETURNING id, allocated_at`, [errand_id, staff_user_id, company_id, requester_id]);
        // Update errand status if this is the first allocation
        await db.query(`UPDATE errands SET status = 'allocated_to_staff'
       WHERE id = $1 AND status IN ('pending', 'open')`, [errand_id]);
        // Audit log
        await db.query(`INSERT INTO allocation_audit_log (errand_id, staff_user_id, company_id, allocated_by_user_id, action, details)
       VALUES ($1, $2, $3, $4, 'allocated', $5)`, [
            errand_id,
            staff_user_id,
            company_id,
            requester_id,
            `Allocated to ${staffDetail.display_name}${vulnCheck.vulnerabilityType
...
router.get('/suggest', authMiddleware, async (req, res) => {
    try {
        const { errand_id, company_id } = req.query;
        ...
        // Get errand details
        const errandRes = await db.query(`SELECT title, description FROM errands WHERE id = $1`, [errand_id]);
        if (!errandRes.rows.length) {
            return res.status(404).json({ success: false, error: 'Errand not found' });
        }
        const errand = errandRes.rows[0];
        // Get all active staff in company
        const staffRes = await db.query(`SELECT u.id, u.display_name, u.email, u.vulnerability_type, u.is_vulnerable_person,
       ...
       ORDER BY csa.assigned_at DESC`, [company_id]);
        // Check each staff member for vulnerability restrictions
        const suggestions = await Promise.all(staffRes.rows.map(async (staff) => {
            const vulnCheck = await checkVulnerabilityRestrictions(staff.id, errand.title, errand.description);
            return {
                id: staff.id,
...
router.post('/unallocate', authMiddleware, async (req, res) => {
    try {
        const { errand_id, staff_user_id, company_id } = req.body;
        const requester_id = parseInt(req.userId || '0', 10);
        if (!errand_id || !staff_user_id || !company_id) {
       ...
       WHERE company_id = $1 AND user_id = $2 AND status = 'active'`, [company_id, requester_id]);
        ...
        // Remove allocation
        await db.query(`DELETE FROM errand_staff_allocations
       WHERE errand_id = $1 AND staff_user_id = $2 AND company_id = $3`, [errand_id, staff_user_id, company_id]);
        // Audit log
        await db.query(`INSERT INTO allocation_audit_log (errand_id, staff_user_id, company_id, allocated_by_user_id, action, details)
       VALUES ($1, $2, $3, $4, 'unallocated', 'Allocation removed')`, [errand_id, staff_user_id, company_id, requester_id]);
        return res.json({
            success: true,
