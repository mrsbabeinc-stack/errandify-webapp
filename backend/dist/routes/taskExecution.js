...
router.post('/:id/start', authMiddleware, async (req, res) => {
    try {
        ...
        // Get task details
        const errandResult = await db.query(`SELECT e.*, ea.doer_id, ea.status as assignment_status
       FROM errands e
       LEFT JOIN errand_assignments ea ON e.id = ea.errand_id AND ea.doer_id = $1
       WHERE e.id = $2`, [doerId, taskId]);
        ...
        await db.query('UPDATE errand_assignments SET status = $1 WHERE errand_id = $2 AND doer_id = $3', ['in_progress', taskId, doerId]);
        // Update errand status if needed
        if (errand.status === 'confirmed') {
            await db.query('UPDATE errands SET status = $1 WHERE id = $2', ['in_progress', taskId]);
        }
        // Notify asker
        await createNotification(errand.asker_id, 'task_started', 'Errand Started', `${errand.errand_id}: Started`, null).catch(console.error);
        res.json({
            success: true,
            data: {
                taskId,
...
router.post('/:id/upload-photo', authMiddleware, async (req, res) => {
    try {
        const taskId = parseInt(req.params.id, 10);
        const doerId = parseInt(req.userId || '0', 10);
        ...
        // Verify doer is assigned
        const assignmentResult = await db.query('SELECT * FROM errand_assignments WHERE errand_id = $1 AND doer_id = $2', [taskId, doerId]);
        if (assignmentResult.rows.length === 0) {
            return res.status(403).json({ error: 'You are not assigned to this task' });
        }
        // Save photo
        const photoResult = await db.query(`INSERT INTO task_photos (errand_id, doer_id, photo_url, caption, uploaded_at)
       VALUES ($1, $2, $3, $4, NOW())
       RETURNING id, uploaded_at`, [taskId, doerId, photoUrl, caption || null]);
        // Get task for notification
        const errandResult = await db.query('SELECT asker_id, title FROM errands WHERE id = $1', [taskId]);
        const errand = errandResult.rows[0];
        // Notify asker
        await createNotification(errand.asker_id, 'task_photo_received', '📸 Photo Received', `Doer uploaded proof of work for "${errand.title}"`, null).catch(console.error);
        res.json({
            success: true,
            data: {
...
router.post('/:id/complete', authMiddleware, async (req, res) => {
    try {
        ...
        // Get task
        const errandResult = await db.query('SELECT * FROM errands WHERE id = $1', [taskId]);
        if (errandResult.rows.length === 0) {
            return res.status(404).json({ error: 'Task not found' });
        }
        const errand = errandResult.rows[0];
       ...
       SET status = $1,
           completion_notes = $2,
           completed_at = NOW()
       WHERE id = $3`, [newStatus, notes || null, taskId]);
        // Get doer info for notification
        const assignmentResult = await db.query(`SELECT ea.doer_id, u.display_name
       FROM errand_assignments ea
       JOIN users u ON ea.doer_id = u.id
       WHERE ea.errand_id = $1`, [taskId]);
        if (assignmentResult.rows.length > 0) {
            const assignment = assignmentResult.rows[0];
            // Notify doer
            if (approved !== false) {
                await createNotification(assignment.doer_id, 'task_completed', 'Errand Completed', `${errand.errand_id}: Your work on "${errand.title}" was approved. Payment will be released after 48h.`, null).catch(console.error);
            }
            else {
                await createNotification(assignment.doer_id, 'task_needs_revision', 'Needs Revision', `${errand.errand_id}: Your work on "${errand.title}" needs revision: ${notes || 'Please redo the work'}`, null).catch(console.error);
            }
        }
        res.json({
            success: true,
...
router.get('/:id/status', authMiddleware, async (req, res) => {
    try {
       ...
       WHERE e.id = $1 AND (e.asker_id = $2 OR ea.doer_id = $2)`, [taskId, userId]);
...
router.get('/:id/photos', authMiddleware, async (req, res) => {
    try {
        ...
        const userId = parseInt(req.userId || '0', 10);
        // Verify access
        const errandResult = await db.query('SELECT asker_id FROM errands WHERE id = $1', [taskId]);
        if (errandResult.rows.length === 0) {
            return res.status(404).json({ error: 'Task not found' });
        }
        const errand = errandResult.rows[0];
        // Can be viewed by asker or the assigned doer
        const assignmentResult = await db.query('SELECT doer_id FROM errand_assignments WHERE errand_id = $1 AND doer_id = $2', [taskId, userId]);
        ...
        // Get photos
        const photosResult = await db.query(`SELECT id, photo_url as url, caption, uploaded_at as uploadedAt, doer_id
       FROM task_photos
       WHERE errand_id = $1
