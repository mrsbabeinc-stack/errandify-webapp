...
import { getUserReferralCode, trackReferralJoin, trackReferralFirstJob, getReferralStats, getReferralRewards, } from '../services/referralService.js';
const router = Router();
/**
 ...
 */
router.post('/track-join', authMiddleware, async (req, res) => {
    try {
 ...
 * Body: { referrer_id }
 */
router.post('/track-first-job', authMiddleware, async (req, res) => {
    try {
        const referredUserId = parseInt(req.userId || '0', 10);
