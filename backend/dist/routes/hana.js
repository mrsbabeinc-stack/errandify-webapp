...
router.post('/chat/hana', authMiddleware, async (req, res) => {
    try {
...
// SOS Emergency Help
router.post('/chat/hana/sos', authMiddleware, async (req, res) => {
    try {
...
router.post('/chat/hana/customer-service', async (req, res) => {
    try {
...
router.post('/chat/hana/speak', async (req, res) => {
    try {
        ...
        try {
            const qwenTtsResponse = await axios.post('https://dashscope-intl.aliyuncs.com/api/v1/services/aigc/text2speech/synthesis', {
                model: 'cosyvoice-v1',
                input: {
                    text: text,
                },
...
const fallbackToGTTS = async (text, lang, res) => {
    try {
