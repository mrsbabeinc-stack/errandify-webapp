...
router.get('/:userId', async (req, res) => {
    try {
       ...
       WHERE id = $1`, [userId]);
        ...
        // Get rating details
        const ratingResult = await db.query(`SELECT
       ...
       WHERE rated_user_id = $1`, [userId]);
        ...
        const reviewsResult = await db.query(`SELECT
         r.id,
         r.rating,
         r.comment,
         r.created_at,
       ...
       WHERE ea.doer_id = $1 OR e.asker_id = $1`, [userId]);
...
router.put('/me/update', authMiddleware, async (req, res) => {
    try {
        ...
        const updateResult = await db.query(`UPDATE users
       SET
       ...
       RETURNING
         ...
         average_rating,
         total_ratings,
         kyc_status`, [displayName || null, bio || null, skills ? JSON.stringify(skills) : null, profileImageUrl || null, phone || null, userId]);
