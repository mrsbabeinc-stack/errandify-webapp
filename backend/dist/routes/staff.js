...
router.post('/invite', authMiddleware, async (req, res) => {
    try {
        ...
        const adminCheck = await db.query(`SELECT * FROM company_staff_assignments
       WHERE company_id = $1 AND user_id = $2 AND role IN ('manager', 'supervisor')`, [company_id, inviting_user_id]);
        ...
        if (user_id) {
            ...
            await db.query(`INSERT INTO staff_invitations (company_id, user_id, email, invite_code, invited_by_user_id, status)
         VALUES ($1, $2, $3, $4, $5, 'pending')
         ON CONFLICT (invite_code) DO NOTHING`, [company_id, user_id, email, invite_code, inviting_user_id]);
            return res.json({
                success: true,
            ...
            await db.query(`INSERT INTO staff_invitations (company_id, email, invite_code, invited_by_user_id, status)
         VALUES ($1, $2, $3, $4, 'pending')
         ON CONFLICT (invite_code) DO NOTHING`, [company_id, email, invite_code, inviting_user_id]);
            return res.json({
                success: true,
...
router.get('/invitations', authMiddleware, async (req, res) => {
    try {
      ...
      ORDER BY si.created_at DESC`, [user_id]);
        res.json({
            success: true,
...
router.post('/accept-invite', authMiddleware, async (req, res) => {
    try {
        ...
        // Get invitation
        const inviteRes = await db.query(`SELECT * FROM staff_invitations WHERE invite_code = $1 AND status = 'pending' AND expires_at > NOW()`, [invite_code]);
        if (inviteRes.rows.length === 0) {
            return res.status(404).json({ success: false, error: 'Invalid or expired invitation' });
        }
        const invitation = inviteRes.rows[0];
        // Update invitation as accepted
        await db.query(`UPDATE staff_invitations SET status = 'accepted', user_id = $1, accepted_at = NOW() WHERE invite_code = $2`, [user_id, invite_code]);
        // Create or update assignment
        await db.query(`INSERT INTO company_staff_assignments (company_id, user_id, role, status)
       VALUES ($1, $2, 'staff', 'active')
       ON CONFLICT (company_id, user_id) DO UPDATE SET status = 'active'`, [invitation.company_id, user_id]);
        res.json({
            success: true,
...
router.post('/decline-invite', authMiddleware, async (req, res) => {
    try {
        ...
        await db.query(`UPDATE staff_invitations SET status = 'declined', declined_at = NOW() WHERE invite_code = $1`, [invite_code]);
        res.json({
            success: true,
...
router.get('/members', authMiddleware, async (req, res) => {
    try {
        ...
        const members = await db.query(`SELECT
        csa.id,
        csa.user_id,
        csa.role,
        csa.status,
...
router.delete('/remove', authMiddleware, async (req, res) => {
    try {
        ...
        const adminCheck = await db.query(`SELECT * FROM company_staff_assignments
       WHERE company_id = $1 AND user_id = $2 AND role IN ('manager', 'supervisor')`, [company_id, requester_id]);
        if (adminCheck.rows.length === 0) {
        ...
        await db.query(`UPDATE company_staff_assignments
       SET status = 'removed', removed_at = NOW()
       WHERE company_id = $1 AND user_id = $2`, [company_id, user_id]);
        res.json({
            success: true,
...
router.get('/search', authMiddleware, async (req, res) => {
    try {
      ...
      LIMIT 20`, [searchPattern, company_id]);
        res.json({
            success: true,
            data: users.rows
        });
