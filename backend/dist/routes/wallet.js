...
router.get('/', authMiddleware, async (req, res) => {
    try {
        ...
        const earningsResult = await db.query(`SELECT
       ...
       WHERE ea.doer_id = $1`, [userId]);
        const earnings = earningsResult.rows[0];
        // Get pending payouts and spent amount (for askers)
        const spentResult = await db.query(`SELECT
...
router.get('/transactions', authMiddleware, async (req, res) => {
    try {
        ...
        const doerTransactions = await db.query(`SELECT
         'earning' as type,
         e.id as task_id,
         e.title,
...
router.get('/breakdown', authMiddleware, async (req, res) => {
    try {
        ...
        const categoryResult = await db.query(`SELECT
         e.category,
         COUNT(*) as task_count,
       ...
       GROUP BY e.category
       ORDER BY category_earnings DESC`, [userId]);
        // Get earnings by month (last 6 months)
        const monthlyResult = await db.query(`SELECT
...
router.get('/search-users', authMiddleware, async (req, res) => {
    try {
        ...
        const result = await db.query(`SELECT id, display_name, user_id
       FROM users
...
router.post('/gift-points', authMiddleware, async (req, res) => {
    try {
        ...
        const recipientResult = await db.query(`SELECT id FROM users WHERE id = $1`, [recipientId]);
        if (recipientResult.rows.length === 0) {
            return res.status(404).json({ error: 'Recipient not found' });
        }
        // Update sender's points (deduct)
        await db.query(`UPDATE users SET errandify_points = errandify_points - $1 WHERE id = $2`, [pointsAmount, senderId]);
...
router.get('/point-history', authMiddleware, async (req, res) => {
    try {
        const userId = parseInt(req.userId || '0', 10);
        // Fetch all point transactions for this user (ordered by most recent first)
        const result = await db.query(`SELECT id, sender_id, recipient_id, user_id, points, type, description, created_at
...
router.get('/my-vouchers', authMiddleware, async (req, res) => {
    try {
        ...
        const result = await db.query(`SELECT id, user_id, points, type, description, created_at
       FROM point_transactions
...
router.get('/rewards', async (req, res) => {
    try {
        const result = await db.query(`SELECT id, name, description, category, cost_points, icon, status, created_at
       FROM rewards
       WHERE status = 'active'
...
router.post('/rewards', authMiddleware, async (req, res) => {
    try {
        ...
        const result = await db.query(`INSERT INTO rewards (name, description, category, cost_points, icon, status)
       VALUES ($1, $2, $3, $4, $5, 'active')
       RETURNING *`, [name, description || null, category || 'other', cost_points, icon || '🎁']);
        res.status(201).json({
            success: true,
            data: result.rows[0],
        });
...
router.put('/rewards/:id', authMiddleware, async (req, res) => {
    try {
        ...
        const result = await db.query(`UPDATE rewards
       SET name = COALESCE($1, name),
           ...
           updated_at = NOW()
       WHERE id = $7
       RETURNING *`, [name, description, category, cost_points, icon, status, rewardId]);
...
router.delete('/rewards/:id', authMiddleware, async (req, res) => {
    try {
        ...
        const result = await db.query(`UPDATE rewards SET status = 'inactive', updated_at = NOW() WHERE id = $1 RETURNING *`, [rewardId]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'Reward not found' });
        }
        res.json({
...
router.post('/award-ep', authMiddleware, async (req, res) => {
    try {
        ...
        // Award to asker
        if (askerId) {
            await db.query(`UPDATE users SET errandify_points = errandify_points + $1, updated_at = NOW() WHERE id = $2`, [TOTAL_EP, askerId]);
            // Log transaction
            await db.query(`INSERT INTO wallet_transactions (user_id, type, amount, description, errand_id, created_at)
         VALUES ($1, $2, $3, $4, $5, NOW())`, [askerId, 'ep_earned', TOTAL_EP, `Errand completion + rating bonus`, errandId]);
        }
        // Award to doer
        if (doerId) {
            await db.query(`UPDATE users SET errandify_points = errandify_points + $1, updated_at = NOW() WHERE id = $2`, [TOTAL_EP, doerId]);
            // Log transaction
            await db.query(`INSERT INTO wallet_transactions (user_id, type, amount, description, errand_id, created_at)
         VALUES ($1, $2, $3, $4, $5, NOW())`, [doerId, 'ep_earned', TOTAL_EP, `Errand completion + rating bonus`, errandId]);
        }
        res.json({
            success: true,
...
router.post('/award-ep-bonus', authMiddleware, async (req, res) => {
    try {
        ...
        // Award bonus EP
        await db.query(`UPDATE users SET errandify_points = errandify_points + $1, updated_at = NOW() WHERE id = $2`, [bonus, userId]);
        // Log transaction
        await db.query(`INSERT INTO wallet_transactions (user_id, type, amount, description, errand_id, created_at)
       VALUES ($1, $2, $3, $4, $5, NOW())`, [userId, 'ep_earned', bonus, `${reason} bonus`, errandId]);
        res.json({
            success: true,
