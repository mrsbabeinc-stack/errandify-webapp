...
function generateOfferId(category) {
...
router.post('/', authMiddleware, async (req, res) => {
    try {
        ...
        const errandResult = await db.query('SELECT id, status, asker_id FROM errands WHERE id = $1', [task_id]);
        ...
        const existingBidResult = await db.query('SELECT id, status FROM bids WHERE errand_id = $1 AND doer_id = $2', [task_id, doerId]);
        let bid;
        if (existingBidResult.rows.length > 0) {
            ...
            await db.query('UPDATE bids SET amount = $1, note = $2, updated_at = NOW() WHERE id = $3', [parseFloat(amount), note || null, bidId]);
            const updated = await db.query('SELECT * FROM bids WHERE id = $1', [bidId]);
            bid = updated.rows[0];
        }
        else {
            // Check if doer has a rejected bid for this errand
            const rejectedBidResult = await db.query('SELECT id FROM bids WHERE errand_id = $1 AND doer_id = $2 AND status = $3', [task_id, doerId, 'rejected']);
            ...
            const errandCategoryResult = await db.query('SELECT title, errand_id, category FROM errands WHERE id = $1', [task_id]);
            ...
            const result = await db.query('INSERT INTO bids (errand_id, doer_id, amount, note, status, offer_id) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *', [task_id, doerId, parseFloat(amount), note || null, 'pending', offerId]);
            bid = result.rows[0];
        ...
        if (selectedSessions.length > 0) {
            try {
                ...
                const sessionResult = await db.query(`SELECT id, instance_number FROM recurring_sessions
           WHERE parent_errand_id = $1 AND instance_number = ANY($2::int[])`, [task_id, selectedSessions]);
                // Insert bid-session mappings
                for (const session of sessionResult.rows) {
                    await db.query('INSERT INTO bid_sessions (bid_id, session_id) VALUES ($1, $2) ON CONFLICT DO NOTHING', [bid.id, session.id]);
                }
...
router.get('/task/:taskId', authMiddleware, async (req, res) => {
    try {
        ...
        const errandResult = await db.query('SELECT asker_id FROM errands WHERE id = $1', [parsedTaskId]);
        if (errandResult.rows.length === 0) {
            return res.status(404).json({ error: 'Looks like this errand got taken care of or the neighbour changed their mind.' });
        }
        ...
        const bidsResult = await db.query(`SELECT b.id, b.errand_id as taskId, b.doer_id as doerId, u.display_name as doerName,
       ...
       WHERE b.errand_id = $1
       ORDER BY b.created_at DESC`, [parsedTaskId]);
...
router.post('/:id/accept', authMiddleware, async (req, res) => {
    try {
        ...
        const bidResult = await db.query(`SELECT b.*, e.asker_id FROM bids b
       ...
       WHERE b.id = $1`, [id]);
        ...
        await db.query('UPDATE bids SET status = $1 WHERE id = $2', ['accepted', id]);
        // Reject all other bids for this errand
        await db.query('UPDATE bids SET status = $1 WHERE errand_id = $2 AND id != $3', ['rejected', bid.errand_id, id]);
        // Notify other bidders that the job is closed
        try {
            const otherBids = await db.query('SELECT DISTINCT doer_id FROM bids WHERE errand_id = $1 AND id != $2', [bid.errand_id, id]);
        ...
        const updateResult = await db.query('UPDATE errands SET status = $1, accepted_bid_id = $2, confirmation_expires_at = NOW() + INTERVAL \'24 hours\' WHERE id = $3', ['confirmed', id, bid.errand_id]);
        ...
        try {
            await db.query(`INSERT INTO errand_assignments (errand_id, doer_id, status, created_at)
         VALUES ($1, $2, $3, NOW())
         ON CONFLICT (errand_id, doer_id) DO UPDATE
         SET status = $3`, [bid.errand_id, bid.doer_id, 'accepted']);
...
router.post('/:id/reject', authMiddleware, async (req, res) => {
    try {
       ...
       WHERE b.id = $1`, [id]);
        ...
        await db.query('UPDATE bids SET status = $1, rejection_reason = $2, custom_reason = $3, rejected_at = NOW() WHERE id = $4', ['rejected', reason, custom_reason || null, id]);
        // Get updated bid
        ...
        try {
            ...
            const errandData = await db.query('SELECT title FROM errands WHERE id = $1', [bid.errand_id]);
            const errandTitle = errandData.rows[0]?.title || 'Your task';
            await db.query(`INSERT INTO notifications (user_id, type, title, message, related_errand_id, created_at, is_read)
         VALUES ($1, $2, $3, $4, $5, NOW(), false)`, [
                bid.doer_id,
...
router.get('/user/:userId/confidence', async (req, res) => {
    try {
        ...
        // Get user info
        const userResult = await db.query('SELECT id, display_name FROM users WHERE id = $1', [parsedUserId]);
       ...
       WHERE b.doer_id = $1`, [parsedUserId]);
...
router.get('/my-bids', authMiddleware, async (req, res) => {
    try {
        ...
        const now = new Date();
        await db.query(`UPDATE errands
       SET status = 'expired'
       WHERE status = 'open' AND deadline < $1`, [now]);
        // Get all bids for this doer with errand details and rating status
        const bidsResult = await db.query(`SELECT b.*, e.title, e.budget, e.category, e.status as errand_status, e.location, e.full_address, e.postal_code, e.deadline, e.description, e.formatted_id, e.accepted_bid_id, u.alias, u.display_name as asker_display_name,
       ...
       WHERE b.doer_id = $1
       ORDER BY b.created_at DESC`, [doerId]);
...
router.get('/check/:errandId', authMiddleware, async (req, res) => {
    try {
        const { errandId } = req.params;
        const doerId = parseInt(req.userId || '0', 10);
        // Resolve errand ID (accepts both database ID and formatted errand ID)
        let parsedErrandId = null;
        if (/^\d+$/.test(errandId)) {
            ...
            const errandResult = await db.query('SELECT id FROM errands WHERE errand_id = $1', [errandId]);
            if (errandResult.rows.length > 0) {
        ...
        const result = await db.query(`SELECT id, amount, status, note FROM bids
       WHERE errand_id = $1 AND doer_id = $2
       LIMIT 1`, [parsedErrandId, doerId]);
...
router.put('/:id/confirm', authMiddleware, async (req, res) => {
    try {
        ...
        // Get the bid
        const bidResult = await db.query('SELECT id, doer_id, errand_id, status FROM bids WHERE id = $1', [bidId]);
        ...
        await db.query('UPDATE bids SET status = $1 WHERE id = $2', ['confirmed', bidId]);
        console.log('[Bids] Bid updated to confirmed');
        // Close all other bids for this errand (set status to 'closed')
        await db.query('UPDATE bids SET status = $1 WHERE errand_id = $2 AND id != $3', ['closed', bid.errand_id, bidId]);
        // Update errand status to 'confirmed' (awaiting doer to start)
        await db.query('UPDATE errands SET status = $1 WHERE id = $2', ['confirmed', bid.errand_id]);
        // Notify other bidders that the job has started and their offers are closed
        try {
            const otherBids = await db.query('SELECT DISTINCT doer_id FROM bids WHERE errand_id = $1 AND id != $2 AND status = $3', [bid.errand_id, bidId, 'closed']);
            const errandData = await db.query('SELECT title FROM errands WHERE id = $1', [bid.errand_id]);
            const errandTitle = errandData.rows[0]?.title || 'A task';
            for (const otherBid of otherBids.rows) {
                await db.query(`INSERT INTO notifications (user_id, type, title, message, related_errand_id, created_at, is_read)
...
router.put('/:id/accept-sessions', authMiddleware, async (req, res) => {
    try {
        const bidId = parseInt(req.params.id, 10);
        ...
        // Get the bid
        const bidResult = await db.query(`SELECT b.id, b.errand_id, b.doer_id, e.asker_id
       FROM bids b
       JOIN errands e ON b.errand_id = e.id
       WHERE b.id = $1`, [bidId]);
        if (bidResult.rows.length === 0) {
        ...
        const sessionsResult = await db.query(`SELECT id, instance_number, errand_id FROM recurring_sessions
       WHERE parent_errand_id = $1 AND instance_number = ANY($2::int[])`, [bid.errand_id, selectedSessions]);
        ...
        for (const session of sessionsToConfirm) {
            await db.query('UPDATE errands SET status = $1 WHERE id = $2', ['confirmed', session.errand_id]);
            // Create bid record for this specific session instance
            // (This links the doer to this specific session)
            const bidForSessionResult = await db.query(`SELECT id FROM bids WHERE errand_id = $1 AND doer_id = $2 LIMIT 1`, [session.errand_id, bid.doer_id]);
            if (bidForSessionResult.rows.length === 0) {
                // Create a new bid for this session if one doesn't exist
                await db.query('INSERT INTO bids (errand_id, doer_id, amount, status) VALUES ($1, $2, $3, $4)', [session.errand_id, bid.doer_id, bid.amount, 'confirmed']);
            }
            else {
                // Update existing bid to confirmed
                await db.query('UPDATE bids SET status = $1 WHERE id = $2', ['confirmed', bidForSessionResult.rows[0].id]);
            }
        }
...
router.get('/recurring/:errandId', authMiddleware, async (req, res) => {
    try {
        const errandId = parseInt(req.params.errandId, 10);
        const currentUserId = parseInt(req.userId || '0', 10);
        // Verify this is a recurring errand and user is the asker
        const errandResult = await db.query('SELECT id, asker_id, is_recurring FROM errands WHERE id = $1', [errandId]);
        if (errandResult.rows.length === 0) {
            return res.status(404).json({ error: 'Looks like this errand got taken care of or the neighbour changed their mind.' });
        }
        const errand = errandResult.rows[0];
        ...
        // Get all sessions for this recurring errand
        const sessionsResult = await db.query(`SELECT rs.id, rs.instance_number, rs.errand_id, rs.scheduled_date,
              e.title, e.status, e.budget
       FROM recurring_sessions rs
       JOIN errands e ON rs.errand_id = e.id
       WHERE rs.parent_errand_id = $1
       ORDER BY rs.instance_number ASC`, [errandId]);
        const sessions = sessionsResult.rows;
        // Get all bids for all sessions
        const bidsResult = await db.query(`SELECT DISTINCT b.id, b.doer_id, b.amount, b.note, b.status, b.created_at,
       ...
       ORDER BY b.created_at DESC`, [errandId]);
        const bids = bidsResult.rows;
        // Map bids to their sessions
        const bidSessionsResult = await db.query(`SELECT bs.bid_id, rs.instance_number
       FROM bid_sessions bs
       JOIN recurring_sessions rs ON bs.session_id = rs.id
       WHERE rs.parent_errand_id = $1`, [errandId]);
        const bidSessionMap = {};
...
router.post('/:bidId/mark-viewed', authMiddleware, async (req, res) => {
    try {
...
router.post('/mark-errand-viewed/:errandId', authMiddleware, async (req, res) => {
    try {
        const { errandId } = req.params;
        const userId = parseInt(req.userId || '0', 10);
        if (!errandId) {
        ...
        const errandResult = await db.query('SELECT asker_id FROM errands WHERE id = $1 OR formatted_id = $1', [errandId]);
        if (errandResult.rows.length === 0) {
        ...
        const updateResult = await db.query(`UPDATE bids
       SET viewed_at = NOW(), viewed_by_asker = TRUE
       WHERE task_id = $1 AND viewed_at IS NULL`, [errand.id]);
