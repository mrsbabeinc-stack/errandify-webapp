...
function generateErrandId(category) {
...
async function resolveErrandId(idParam) {
    ...
    try {
        const result = await db.query('SELECT id FROM errands WHERE errand_id = $1', [idParam]);
...
router.get('/', authMiddleware, async (req, res) => {
    try {
        ...
        const errandsWithAskerInfo = await Promise.all(result.rows.map(async (errand) => {
            const askerResult = await db.query('SELECT display_name FROM users WHERE id = $1', [errand.asker_id]);
            // Get doer name from confirmed bid if exists
            let doerName = 'Doer';
            const bidResult = await db.query(`SELECT u.display_name FROM bids b
           ...
           FROM bids
           WHERE errand_id = $1 AND status IN ('pending', 'accepted', 'rejected_resubmitted')`, [errand.id]);
...
router.get('/:id', authMiddleware, async (req, res) => {
    try {
        ...
        if (errand.status === 'cancelled' && userId) {
            ...
            const hasBid = await db.query('SELECT id FROM bids WHERE errand_id = $1 AND doer_id = $2 LIMIT 1', [errandDatabaseId, userId]);
        ...
        // Get asker info
        const askerResult = await db.query('SELECT display_name, alias, mobile FROM users WHERE id = $1', [errand.asker_id]);
        ...
        const bidResult = await db.query(`SELECT doer_id FROM bids
       ...
       LIMIT 1`, [errand.id]);
        ...
        if (bidResult.rows[0]) {
            ...
            // Get doer info
            const doerResult = await db.query('SELECT display_name, alias FROM users WHERE id = $1', [doerId]);
...
router.post('/', authMiddleware, async (req, res) => {
    try {
        ...
        let postalCode = postal_code || null;
        ...
        try {
            ...
            const errandResult = await db.query(`INSERT INTO errands (asker_id, title, description, category, location, full_address, postal_code, budget, deadline, is_recurring, recurring_schedule, status, formatted_id)
         ...
         RETURNING id, formatted_id, title, description, category, status, budget, deadline, is_recurring, recurring_schedule, created_at`, [
            ...
            const allFieldsLog = await db.query('SELECT id, formatted_id, title, description, location, postal_code, category, budget, deadline, status, is_recurring FROM errands WHERE id = $1', [errand.id]);
            ...
            try {
                ...
                const notifyResult = await db.query(`SELECT DISTINCT u.id as doer_id, u.display_name
           FROM users u
           WHERE u.role = 'doer'
           AND (
             ...
             (
               ...
               NOT EXISTS (
                ...
                for (const doer of notifyResult.rows) {
                    await db.query(`INSERT INTO notifications (user_id, type, title, message, related_id, related_type, created_at)
...
router.post('/:id/complete', authMiddleware, async (req, res) => {
    try {
        ...
        const errandResult = await db.query('SELECT status, accepted_bid_id, stripe_payment_intent_id FROM errands WHERE id = $1', [id]);
        ...
        try {
            const completedCount = await db.query(`SELECT COUNT(*) as count FROM errands
         WHERE doer_id = $1 AND status IN ('completed', 'completed_confirmed', 'completed_unconfirmed')`, [doerId]);
            ...
            if (completedCount.rows[0].count === 1) {
                ...
                const referralInfo = await db.query(`SELECT referrer_id FROM referral_tracking
           WHERE referred_user_id = $1 AND status = 'joined'`, [doerId]);
                if (referralInfo.rows.length > 0) {
                    const referrerId = referralInfo.rows[0].referrer_id;
                    const firstJobBonus = 50;
                    // Update tracking status
                    await db.query(`UPDATE referral_tracking
             SET status = 'first_job_completed', first_job_completed_at = NOW()
             WHERE referred_user_id = $1`, [doerId]);
                    // Award first job bonus
                    await db.query(`INSERT INTO referral_rewards (referrer_id, reward_type, points_amount)
             VALUES ($1, $2, $3)`, [referrerId, 'first_job', firstJobBonus]);
                    // Update referrer's points
                    await db.query(`UPDATE users SET errandify_points = errandify_points + $1 WHERE id = $2`, [firstJobBonus, referrerId]);
                    // Log EP transaction
                    await db.query(`INSERT INTO ep_transactions (user_id, transaction_type, points_change, description, created_at)
             SELECT $1, 'referral_first_job', $2, 'Referral first job bonus - ' || $3 || ' completed first errand', NOW()
             FROM users WHERE id = $1`, [referrerId, firstJobBonus, doerName]);
                    // Send notification to referrer
                    try {
                        await db.query(`INSERT INTO notifications (user_id, type, title, message, data, created_at)
            ...
            if (completedCount.rows[0].count === 10) {
                const loyaltyInfo = await db.query(`SELECT referrer_id FROM referral_tracking
           WHERE referred_user_id = $1 AND status IN ('first_job_completed', 'loyal')`, [doerId]);
                if (loyaltyInfo.rows.length > 0) {
                    ...
                    await db.query(`UPDATE referral_tracking
             SET status = 'loyal' WHERE referred_user_id = $1`, [doerId]);
                    // Award loyalty bonus
                    await db.query(`INSERT INTO referral_rewards (referrer_id, reward_type, points_amount)
             VALUES ($1, $2, $3)`, [referrerId, 'loyalty', loyaltyBonus]);
                    // Update referrer's points
                    await db.query(`UPDATE users SET errandify_points = errandify_points + $1 WHERE id = $2`, [loyaltyBonus, referrerId]);
                    // Log EP transaction
                    await db.query(`INSERT INTO ep_transactions (user_id, transaction_type, points_change, description, created_at)
             SELECT $1, 'referral_loyalty', $2, 'Referral loyalty bonus - ' || $3 || ' reached 10 errands', NOW()
             FROM users WHERE id = $1`, [referrerId, loyaltyBonus, doerName]);
                    // Send notification to referrer
                    try {
                        await db.query(`INSERT INTO notifications (user_id, type, title, message, data, created_at)
               VALUES ($1, $2, $3, $4, $5, NOW())`, [
...
router.put('/:id', authMiddleware, async (req, res) => {
    try {
        ...
        const checkResult = await db.query('SELECT asker_id, status FROM errands WHERE id = $1', [id]);
...
router.post('/check-duplicate', authMiddleware, async (req, res) => {
    try {
        ...
        const result = await db.query(`SELECT id, title, category, location, deadline, time, created_at
       FROM errands
       WHERE asker_id = $1
       AND created_at > $2
       AND status != 'cancelled'
       ORDER BY created_at DESC`, [userId, twentyFourHoursAgo.toISOString()]);
...
router.post('/:id/confirm', authMiddleware, async (req, res) => {
    try {
        ...
        const doerId = parseInt(req.userId || '0', 10);
        // Get errand details
        const errandResult = await db.query('SELECT id, status, accepted_bid_id FROM errands WHERE id = $1', [id]);
        if (errandResult.rows.length === 0) {
            return res.status(404).json({ error: 'Errand not found' });
        }
        const errand = errandResult.rows[0];
        ...
        await db.query('UPDATE errands SET status = $1 WHERE id = $2', ['confirmed_awaiting_start', id]);
        res.json({
            success: true,
...
router.post('/:id/start', authMiddleware, async (req, res) => {
    try {
        ...
        const errandResult = await db.query('SELECT id, status FROM errands WHERE id = $1', [errandDatabaseId]);
        if (errandResult.rows.length === 0) {
            return res.status(404).json({ error: 'Errand not found' });
        }
        ...
        await db.query('UPDATE errands SET status = $1 WHERE id = $2', ['in_progress', errandDatabaseId]);
...
router.post('/:id/end', authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const errandResult = await db.query('SELECT id, status FROM errands WHERE id = $1', [id]);
        if (errandResult.rows.length === 0) {
            return res.status(404).json({ error: 'Errand not found' });
        }
        ...
        await db.query('UPDATE errands SET status = $1, job_ended_at = NOW(), dispute_deadline = NOW() + INTERVAL \'48 hours\' WHERE id = $2', ['job_completed', id]);
        res.json({
            success: true,
...
router.post('/:id/reopen', authMiddleware, async (req, res) => {
    try {
        ...
        // Get errand details
        const errandResult = await db.query('SELECT id, status, asker_id, accepted_bid_id, dispute_deadline FROM errands WHERE id = $1', [id]);
        if (errandResult.rows.length === 0) {
            return res.status(404).json({ error: 'Errand not found' });
        }
        const errand = errandResult.rows[0];
        ...
        await db.query('UPDATE errands SET status = $1, reopened_reason = $2, reopened_by = $3 WHERE id = $4', ['in_progress', reason || null, userId, id]);
...
router.post('/:id/raise-dispute', authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = parseInt(req.userId || '0', 10);
        const { reason } = req.body;
        const errandResult = await db.query('SELECT id, status, asker_id, dispute_deadline FROM errands WHERE id = $1', [id]);
        if (errandResult.rows.length === 0) {
            return res.status(404).json({ error: 'Errand not found' });
        }
        const errand = errandResult.rows[0];
        ...
        await db.query('UPDATE errands SET status = $1, dispute_reason = $2 WHERE id = $3', ['disputed', reason || null, id]);
...
router.post('/:id/work-proof', authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        // Store work proof
        await db.query('UPDATE errands SET work_proof_description = $1, work_proof_urls = $2, work_proof_submitted_at = NOW() WHERE id = $3', [proof_description, proof_urls ? JSON.stringify(proof_urls) : null, id]);
        res.json({
            success: true,
...
router.post('/:id/review', authMiddleware, async (req, res) => {
    try {
        ...
        const errandResult = await db.query('SELECT id, status, asker_id FROM errands WHERE id = $1', [id]);
        if (errandResult.rows.length === 0) {
            return res.status(404).json({ error: 'Errand not found' });
        }
        const errand = errandResult.rows[0];
        ...
        // Update review
        await db.query(`UPDATE errands SET ${reviewField} = $1, ${commentField} = $2 WHERE id = $3`, [rating, comment || null, id]);
...
router.post('/:id/cancel', authMiddleware, async (req, res) => {
    try {
        ...
        await db.query('UPDATE errands SET status = $1, cancelled_by = $2, cancellation_reason = $3 WHERE id = $4', ['cancelled', userId, reason || null, id]);
        // Get canceller name for logging
        const cancellerResult = await db.query('SELECT display_name FROM users WHERE id = $1', [userId]);
        ...
        try {
            ...
            const bidsResult = await db.query('SELECT id, doer_id, status FROM bids WHERE errand_id = $1', [id]);
            // Update all bids to cancelled status
            await db.query('UPDATE bids SET status = $1 WHERE errand_id = $2', ['cancelled', id]);
            // Get errand and canceller details
            const errandData = await db.query('SELECT title FROM errands WHERE id = $1', [id]);
...
router.post('/:id/confirm-extension-request', authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = parseInt(req.userId || '0', 10);
        const { reason } = req.body;
        const errandResult = await db.query('SELECT id, status, confirmation_expires_at, asker_id FROM errands WHERE id = $1', [id]);
        if (errandResult.rows.length === 0) {
            return res.status(404).json({ error: 'Errand not found' });
        }
        const errand = errandResult.rows[0];
...
router.post('/:id/confirm-extension-approve', authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = parseInt(req.userId || '0', 10);
        const errandResult = await db.query('SELECT id, status, asker_id FROM errands WHERE id = $1', [id]);
        ...
        await db.query('UPDATE errands SET confirmation_expires_at = confirmation_expires_at + INTERVAL \'24 hours\', confirmation_extended = true WHERE id = $1', [id]);
        res.json({
            success: true,
...
router.get('/disputes/list/all', authMiddleware, async (req, res) => {
    try {
        const userId = parseInt(req.userId || '0', 10);
        // TODO: Check if user is admin
        // For now, return disputes
        const result = await db.query(`SELECT id, title, asker_id, status, dispute_reason, created_at, dispute_deadline
...
router.post('/:id/resolve-dispute', authMiddleware, async (req, res) => {
    try {
        ...
        await db.query('UPDATE errands SET status = $1, dispute_resolution = $2, dispute_resolved_at = NOW(), payment_released_to = $3, payment_percentage = $4 WHERE id = $5', ['completed', resolution, payment_to, amount_percentage || 100, id]);
        res.json({
            success: true,
...
router.get('/recommended', authMiddleware, async (req, res) => {
    try {
        ...
        const prefResult = await db.query('SELECT category_preferences FROM users WHERE id = $1', [userId]);
...
router.get('/:id/recurring', async (req, res) => {
    try {
        ...
        const sessionResult = await db.query(`SELECT parent_errand_id, instance_number
       FROM recurring_sessions
       WHERE errand_id = $1`, [errandId]);
        ...
        // Get parent errand
        const parentResult = await db.query(`SELECT id, title, is_recurring, recurring_schedule FROM errands WHERE id = $1`, [parentId]);
        // Get all instances
        const siblingsResult = await db.query(`SELECT rs.instance_number, rs.errand_id, rs.scheduled_date, e.title, e.status, e.budget
       FROM recurring_sessions rs
       JOIN errands e ON rs.errand_id = e.id
       WHERE rs.parent_errand_id = $1
       ORDER BY rs.instance_number ASC`, [parentId]);
        res.json({
            success: true,
            data: {
...
router.post('/:id/confirm-completion', authMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const userId = parseInt(req.userId || '0', 10);
        // Get errand
        ...
        const updateResult = await db.query('UPDATE errands SET status = $1, confirmed_at = NOW(), updated_at = NOW() WHERE id = $2 RETURNING id, status', ['completed', id]);
