...
router.get('/check', async (req, res) => {
    try {
        ...
        if (/^\d+$/.test(errandIdStr)) {
            ...
            const errandResult = await db.query('SELECT id FROM errands WHERE errand_id = $1', [errandIdStr]);
        ...
        const result = await db.query('SELECT id FROM ratings WHERE errand_id = $1 AND rater_id = $2 LIMIT 1', [parsedErrandId, parseInt(userId)]);
        res.json({
            success: true,
            data: {
...
router.post('/', authMiddleware, async (req, res) => {
    try {
        ...
        const taskResult = await db.query('SELECT id, asker_id, status, title FROM errands WHERE id = $1', [taskId]);
        ...
        const doerResult = await db.query(`SELECT b.doer_id FROM bids b
       ...
       WHERE e.id = $1`, [taskId]);
        ...
        const existingResult = await db.query('SELECT id FROM ratings WHERE errand_id = $1 AND rater_id = $2', [taskId, raterId]);
        ...
        const ratingResult = await db.query(`INSERT INTO ratings (errand_id, rater_id, ratee_id, rating, review_text)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING id, created_at`, [taskId, raterId, ratedUserId, rating, comment || null]);
        console.log('[Rating] Rating inserted successfully:', ratingResult.rows[0]);
        // Get rater info for notification
        const raterResult = await db.query('SELECT display_name FROM users WHERE id = $1', [raterId]);
        const raterName = raterResult.rows[0]?.display_name || 'Community Member';
        // Get rated user info
        const ratedUserResult = await db.query('SELECT email FROM users WHERE id = $1', [ratedUserId]);
        ...
        try {
            ...
            await createNotification(ratedUserId, 'rating_received', notificationTitle, notificationBody, null).catch(console.error);
            ...
            if (isAsker) {
                ...
                await createNotification(ratedUserId, 'rating_encouragement', doerTitle, doerBody, null).catch(console.error);
        ...
        try {
            await db.query('UPDATE errands SET status = $1 WHERE id = $2', ['rated', taskId]);
...
async function updateUserRating(userId) {
    try {
       ...
       WHERE id = $3`, [avg_rating ? parseFloat(avg_rating).toFixed(2) : null, total_ratings, userId]);
    }
    catch (error) {
