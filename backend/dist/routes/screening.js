...
router.post('/declare', authMiddleware, async (req, res) => {
    try {
        ...
        const { cypaConviction, womensCharterConviction, penalCodeConviction, elderAbuseConviction, dishonestyConviction, understoodRestrictions, } = req.body;
        // Validate required fields
        if (cypaConviction === undefined ||
        ...
        const anyConviction = cypaConviction ||
            womensCharterConviction ||
            penalCodeConviction ||
            elderAbuseConviction ||
            dishonestyConviction;
        // Insert screening declaration
        const result = await db.query(`INSERT INTO screening_declarations (
        user_id,
        cypa_conviction,
      ...
      RETURNING id`, [
       ...
       WHERE id = $1`, [userId, anyConviction]);
...
router.get('/declarations/:userId', authMiddleware, async (req, res) => {
    try {
...
router.get('/restrictions', authMiddleware, async (req, res) => {
    try {
       ...
       ORDER BY rc.category_name`, [userId]);
        res.json({
            success: true,
            data: {
...
router.get('/categories/accessible', authMiddleware, async (req, res) => {
    try {
        const userId = parseInt(req.userId || '0', 10);
        // Get all categories
        const allCategoriesResult = await db.query(`SELECT DISTINCT category FROM errands WHERE category IS NOT NULL ORDER BY category`);
        // Get user's restrictions
        const restrictionsResult = await db.query(`SELECT rc.category_name
       FROM user_category_restrictions ucr
       JOIN restricted_categories rc ON ucr.restricted_category_id = rc.id
       WHERE ucr.user_id = $1
       AND ucr.is_active = true
       AND (ucr.restriction_end IS NULL OR ucr.restriction_end > NOW())`, [userId]);
...
router.patch('/restrictions/:categoryId', authMiddleware, async (req, res) => {
    try {
        const userId = parseInt(req.body.userId, 10);
        const categoryId = parseInt(req.params.categoryId, 10);
        const { reason, restrictionEnd } = req.body;
        // TODO: Add admin check
        const result = await db.query(`INSERT INTO user_category_restrictions (user_id, restricted_category_id, reason, restriction_end)
         VALUES ($1, $2, $3, $4)
         ON CONFLICT (user_id, restricted_category_id) DO UPDATE
         SET reason = $3, restriction_end = $4, is_active = true
         RETURNING id`, [userId, categoryId, reason || 'Manual admin restriction', restrictionEnd || null]);
        res.json({
            success: true,
            data: {
