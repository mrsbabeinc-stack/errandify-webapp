...
router.post('/migrate-notifications', authMiddleware, async (req, res) => {
    try {
        ...
        const fix1 = await db.query(`UPDATE notifications n
       SET 
       ...
       AND n.title IN ('New offer Placed', 'New Offer Placed')`);
        ...
        const fix2 = await db.query(`UPDATE notifications n
       SET 
       ...
       AND n.message NOT LIKE 'ER%-%'`);
        ...
        const fix3 = await db.query(`UPDATE notifications n
       SET 
       ...
       AND n.message NOT LIKE 'ER%-%'`);
        ...
        const fix4Result = await db.query(`SELECT n.id, n.message, e.errand_id
       ...
       LIMIT 100`);
        ...
        for (const row of fix4Result.rows) {
            ...
            if (newMessage !== row.message) {
                await db.query('UPDATE notifications SET message = $1 WHERE id = $2', [newMessage, row.id]);
                fix4Count++;
            }
        }
