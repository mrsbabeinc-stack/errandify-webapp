...
router.get('/profile', authMiddleware, async (req, res) => {
    try {
        const result = await db.query('SELECT id, user_id, display_name, email, mobile, role, formatted_user_id, profile_image_url, alias, bio, certificates, average_rating, total_ratings, criminal_conviction, singpass_id, gender, errandify_points FROM users WHERE id = $1', [req.userId]);
        ...
        if (!user.formatted_user_id) {
            ...
            await db.query('UPDATE users SET formatted_user_id = $1 WHERE id = $2', [formattedUserId, user.id]);
       ...
       WHERE e.status = 'completed' AND (e.asker_id = $1 OR ea.doer_id = $1)`, [req.userId]);
        ...
        const favoritedResult = await db.query(`SELECT COUNT(*) as favorite_count FROM user_favorites WHERE favorite_user_id = $1`, [req.userId]);
...
router.patch('/categories', authMiddleware, async (req, res) => {
    try {
        ...
        const result = await db.query('UPDATE users SET category_preferences = $1 WHERE id = $2 RETURNING id, category_preferences', [JSON.stringify(categories), req.userId]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'User not found' });
        }
        res.json({
...
router.get('/preferences', authMiddleware, async (req, res) => {
    try {
        ...
        const result = await db.query('SELECT notification_preferences, email_frequency, email_preferences FROM users WHERE id = $1', [userId]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'User not found' });
        }
        const user = result.rows[0];
...
router.get('/preferences', authMiddleware, async (req, res) => {
    try {
        ...
        await db.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS notification_preferences JSONB DEFAULT '{}'`);
        const result = await db.query(`SELECT notification_preferences FROM users WHERE id = $1`, [req.userId]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'User not found' });
        }
        const user = result.rows[0];
...
router.patch('/preferences', authMiddleware, async (req, res) => {
    try {
        ...
        const result = await db.query(`UPDATE users SET notification_preferences = $1 WHERE id = $2 RETURNING id`, [notification_preferences, req.userId]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'User not found' });
        }
        res.json({
...
router.post('/category-preferences', authMiddleware, async (req, res) => {
    try {
        const userId = parseInt(req.userId || '0', 10);
        ...
        await db.query('UPDATE users SET category_preferences = $1 WHERE id = $2', [preferredCategories, userId]);
        res.json({
            success: true,
...
router.get('/referral', authMiddleware, async (req, res) => {
    try {
        ...
        const referralStatsResult = await db.query(`SELECT COUNT(*) as referred_count, SUM(CASE WHEN kyc_status = 'completed' THEN 1 ELSE 0 END) as completed_count
       FROM users WHERE referred_by = $1`, [referralCode]);
...
router.post('/favorite/:userId', authMiddleware, async (req, res) => {
    try {
        ...
        try {
            ...
            const existingResult = await db.query(`SELECT id FROM user_favorites WHERE user_id = $1 AND favorite_user_id = $2`, [currentUserId, favoriteUserId]);
            if (existingResult.rows.length > 0) {
                // Remove favorite
                await db.query(`DELETE FROM user_favorites WHERE user_id = $1 AND favorite_user_id = $2`, [currentUserId, favoriteUserId]);
                return res.json({ success: true, favorited: false, message: 'Removed from favorites' });
            }
            else {
                // Add favorite
                await db.query(`INSERT INTO user_favorites (user_id, favorite_user_id, added_at)
           VALUES ($1, $2, NOW())
           ON CONFLICT DO NOTHING`, [currentUserId, favoriteUserId]);
...
router.get('/favorites', authMiddleware, async (req, res) => {
    try {
        const currentUserId = parseInt(req.userId || '0', 10);
        const result = await db.query(`SELECT favorite_user_id
       FROM user_favorites
       WHERE user_id = $1
       ORDER BY added_at DESC`, [currentUserId]);
...
router.post('/accept-declaration', authMiddleware, async (req, res) => {
    try {
        const userId = parseInt(req.userId || '0', 10);
        ...
        // Save to audit log
        await db.query(`INSERT INTO user_declarations_log (user_id, declaration_type, accepted, accepted_at, ip_address)
       VALUES ($1, $2, $3, $4, $5)`, [userId, 'before_you_get_started', true, acceptedAt || new Date(), req.ip]);
        // Update user table
        await db.query(`UPDATE users SET declaration_accepted = true, declaration_accepted_at = NOW(), account_active = true
       WHERE id = $1`, [userId]);
...
router.get('/declaration-status', authMiddleware, async (req, res) => {
    try {
        const userId = parseInt(req.userId || '0', 10);
        const result = await db.query('SELECT declaration_accepted, declaration_accepted_at, account_active FROM users WHERE id = $1', [userId]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'User not found' });
        }
        const user = result.rows[0];
...
router.post('/certificates', authMiddleware, async (req, res) => {
    try {
        ...
        const result = await db.query('SELECT certificates FROM users WHERE id = $1', [req.userId]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'User not found' });
        }
        ...
        await db.query('UPDATE users SET certificates = $1 WHERE id = $2', [JSON.stringify(updatedCerts), req.userId]);
        res.json({
            success: true,
...
// Delete Certificate
router.delete('/certificates/:title', authMiddleware, async (req, res) => {
    try {
        ...
        const certificates = result.rows[0].certificates || [];
        const updatedCerts = certificates.filter((cert) => cert.title !== decodeURIComponent(title));
        await db.query('UPDATE users SET certificates = $1 WHERE id = $2', [JSON.stringify(updatedCerts), req.userId]);
        res.json({
            success: true,
            data: { certificates: updatedCerts }
...
router.post('/favorites/:userId', authMiddleware, async (req, res) => {
    try {
        ...
        // Check if already favorited
        const existing = await db.query(`SELECT id FROM user_favorites
       WHERE user_id = $1 AND favorite_user_id = $2`, [currentUserId, favoriteUserId]);
        if (existing.rows.length > 0) {
            return res.status(400).json({ error: 'Already favorited.' });
        }
        // Add to favorites
        await db.query(`INSERT INTO user_favorites (user_id, favorite_user_id, added_at)
       VALUES ($1, $2, NOW())`, [currentUserId, favoriteUserId]);
        res.json({
            success: true,
...
router.delete('/favorites/:userId', authMiddleware, async (req, res) => {
    try {
        ...
        const result = await db.query(`DELETE FROM user_favorites
       WHERE user_id = $1 AND favorite_user_id = $2`, [currentUserId, favoriteUserId]);
...
router.get('/deletion-eligibility', authMiddleware, async (req, res) => {
    try {
        const userId = req.userId;
        // Check for active/pending errands (as asker or doer)
        const errandsResult = await db.query(`SELECT COUNT(*) as count FROM errands
       WHERE (asker_id = $1 OR id IN (SELECT errand_id FROM errand_assignments WHERE doer_id = $1))
       AND status IN ('posted', 'bidding', 'accepted', 'in_progress', 'pending_review')`, [userId]);
        const pendingErrands = parseInt(errandsResult.rows[0]?.count || 0);
        // Check for payment holds (money held in escrow)
        const holdsResult = await db.query(`SELECT COUNT(*) as count, COALESCE(SUM(amount), 0) as total_held
       FROM payment_holds
       WHERE user_id = $1 AND status IN ('HOLD', 'PENDING_REVIEW')`, [userId]);
        ...
        const disputesResult = await db.query(`SELECT COUNT(*) as count FROM disputes
       WHERE (asker_id = $1 OR doer_id = $1 OR reporter_id = $1)
       AND status IN ('open', 'pending_review', 'under_investigation')`, [userId]);
        ...
        const paymentsResult = await db.query(`SELECT COUNT(*) as count, COALESCE(SUM(amount), 0) as total_owed
       FROM transactions
       WHERE user_id = $1 AND status IN ('pending', 'overdue')
       AND type IN ('ADVERTISING', 'SUBSCRIPTION', 'PLATFORM_FEE')`, [userId]);
        ...
        const withdrawalsResult = await db.query(`SELECT COUNT(*) as count, COALESCE(SUM(amount), 0) as total_pending
       FROM wallet_transactions
       WHERE user_id = $1 AND type = 'WITHDRAWAL' AND status = 'pending'`, [userId]);
...
router.post('/delete-account', authMiddleware, async (req, res) => {
    try {
        ...
        const eligibilityResult = await db.query(`SELECT COUNT(*) as pending_errands FROM errands
       WHERE (asker_id = $1 OR id IN (SELECT errand_id FROM errand_assignments WHERE doer_id = $1))
       AND status IN ('posted', 'bidding', 'accepted', 'in_progress', 'pending_review')`, [userId]);
        ...
        const holdsResult = await db.query(`SELECT COUNT(*) as count FROM payment_holds
       WHERE user_id = $1 AND status IN ('HOLD', 'PENDING_REVIEW')`, [userId]);
