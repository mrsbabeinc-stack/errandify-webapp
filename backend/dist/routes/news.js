...
router.get('/', async (req, res) => {
    try {
        ...
        if (type === 'all' || type === 'community') {
            const communityNews = await getCommunityNews(parseInt(limit), parseInt(offset), postal_code);
            news.push(...communityNews);
        }
        // Get SGNews
        if (type === 'all' || type === 'singapore') {
            const sgNews = await getSGNews(parseInt(limit), parseInt(offset));
            news.push(...sgNews);
        }
        // Get ErrandifyNews
        if (type === 'all' || type === 'errandify') {
            const errandifyNews = await getErrandifyNews(parseInt(limit), parseInt(offset));
 ...
 */
async function getCommunityNews(limit, offset, postal_code) {
    try {
        const query = `
      SELECT
 ...
 */
async function getSGNews(limit, offset) {
 ...
 */
async function getErrandifyNews(limit, offset) {
    try {
...
router.post('/community', async (req, res) => {
    try {
        ...
        const result = await db.query(`INSERT INTO community_news
       (title, content, category, location, postal_code, posted_by, image, status, created_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, NOW())
       RETURNING *`, [title, content, category, location, postal_code, user.id, image, 'published']);
        res.json({
            success: true,
            data: result.rows[0],
        });
 ...
 */
router.post('/errandify', async (req, res) => {
    try {
        const user = req.user;
        // Check if admin
        if (user?.role !== 'admin') {
        ...
        const result = await db.query(`INSERT INTO errandify_news
       (title, content, category, image, source, status, created_at)
       VALUES ($1, $2, $3, $4, $5, $6, NOW())
       RETURNING *`, [title, content, category, image, source || 'Errandify Team', 'published']);
        res.json({
            success: true,
            data: result.rows[0],
        });
