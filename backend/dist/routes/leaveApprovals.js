...
router.post('/request', authMiddleware, async (req, res) => {
    try {
        const { company_id, leave_type, start_date, end_date, period = 'full-day', reason, notes, is_recurring, recurring_pattern } = req.body;
        ...
        // Get staff info
        const staffRes = await db.query('SELECT display_name FROM users WHERE id = $1', [staff_id]);
        ...
        const leaveRes = await db.query(`INSERT INTO leave_requests (
...
router.get('/requests', authMiddleware, async (req, res) => {
    try {
        ...
        const leaves = await LeaveService.getLeaveRequests(parseInt(company_id), {
            status: status,
            staffId: staff_id ? parseInt(staff_id) : undefined
        });
...
router.put('/request/:id/approve', authMiddleware, async (req, res) => {
    try {
       ...
       WHERE id = $3`, [approver_id, approval_notes || '', id]);
...
router.put('/request/:id/reject', authMiddleware, async (req, res) => {
    try {
       ...
       WHERE id = $3`, [approver_id, rejected_reason || '', id]);
        res.json({
            success: true,
...
router.get('/balance/:staff_id', authMiddleware, async (req, res) => {
    try {
...
router.get('/check', authMiddleware, async (req, res) => {
    try {
        ...
        const isOnLeave = await LeaveService.isStaffOnLeave(parseInt(staff_id), new Date(date));
        res.json({
            success: true,
...
router.get('/hours/:company_id', async (req, res) => {
    try {
...
router.put('/hours/:company_id', authMiddleware, async (req, res) => {
    try {
        const { company_id } = req.params;
        const hoursData = req.body;
        const updated = await OperationHoursService.updateCompanyHours(parseInt(company_id), hoursData);
        if (!updated) {
...
router.get('/check', async (req, res) => {
    try {
        ...
        const check = await OperationHoursService.isWithinOperatingHours(parseInt(company_id), new Date(date_time));
...
router.post('/approvals/request', authMiddleware, async (req, res) => {
    try {
        const { company_id, module, request_type, amount, description, justification } = req.body;
        ...
        // Get requester name
        const userRes = await db.query('SELECT display_name FROM users WHERE id = $1', [requester_id]);
        const requester_name = userRes.rows[0]?.display_name || 'Unknown';
        // Generate request number
        const countRes = await db.query('SELECT COUNT(*) as count FROM approval_requests WHERE company_id = $1 AND EXTRACT(YEAR FROM created_at) = EXTRACT(YEAR FROM NOW())', [company_id]);
        const request_number = `APR-${new Date().getFullYear()}-${String(parseInt(countRes.rows[0].count) + 1).padStart(3, '0')}`;
        // Create request
        const reqRes = await db.query(`INSERT INTO approval_requests (
        company_id, request_number, module, request_type,
        requester_id, requester_name, amount, description, justification
      ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
      RETURNING *`, [company_id, request_number, module, request_type, requester_id, requester_name, amount, description, justification]);
        res.json({
            success: true,
...
router.get('/approvals/requests', authMiddleware, async (req, res) => {
    try {
...
router.put('/approvals/request/:id/step/:step/approve', authMiddleware, async (req, res) => {
    try {
        const { id, step } = req.params;
        const { approver_notes } = req.body;
        const approver_id = parseInt(req.userId || '0', 10);
        // Update step
        await db.query(`UPDATE approval_steps
       SET status = 'approved', approver_id = $1, approver_notes = $2, approved_at = NOW()
       WHERE approval_request_id = $3 AND step_level = $4`, [approver_id, approver_notes || '', id, step]);
        // Check if all steps approved
        const stepsRes = await db.query('SELECT COUNT(*) as total, COUNT(*) FILTER (WHERE status = \'approved\') as approved FROM approval_steps WHERE approval_request_id = $1', [id]);
        const { total, approved } = stepsRes.rows[0];
        // If all approved, update request status
        if (total === approved) {
            await db.query('UPDATE approval_requests SET status = \'approved\', final_decision_date = NOW() WHERE id = $1', [id]);
        }
        else {
            // Move to next step
            await db.query('UPDATE approval_requests SET current_level = current_level + 1 WHERE id = $1', [id]);
        }
        res.json({
            success: true,
...
router.put('/approvals/request/:id/step/:step/reject', authMiddleware, async (req, res) => {
    try {
        const { id, step } = req.params;
        const { approver_notes } = req.body;
        const approver_id = parseInt(req.userId || '0', 10);
       ...
       WHERE approval_request_id = $3 AND step_level = $4`, [approver_id, approver_notes || '', id, step]);
        await db.query('UPDATE approval_requests SET status = \'rejected\', final_decision_date = NOW() WHERE id = $1', [id]);
        res.json({
            success: true,
