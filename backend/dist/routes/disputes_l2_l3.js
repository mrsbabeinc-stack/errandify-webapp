...
import { escalateDisputeToL2, assignDisputeToAgent, getL2DisputesForAgent, resolveL2Dispute, createAppeal, getL3Appeals, resolveAppeal, getSupportQueue, getDashboardStats, } from '../services/escalationService.js';
 ...
 * Auth: Support staff only
 */
router.get('/dashboard/queue', authMiddleware, async (req, res) => {
    try {
        const userId = parseInt(req.userId || '0', 10);
        // Verify user is support staff
        const userResult = await db.query('SELECT role FROM users WHERE id = $1', [userId]);
 ...
 * Auth: Support staff only
 */
router.get('/:id/l2-details', authMiddleware, async (req, res) => {
    try {
        const userId = parseInt(req.userId || '0', 10);
 ...
 * Auth: Support staff only
 */
router.post('/:id/escalate-to-l2', authMiddleware, async (req, res) => {
    try {
        const userId = parseInt(req.userId || '0', 10);
        const disputeId = parseInt(req.params.id, 10);
 ...
 */
router.get('/my-assignments/l2', authMiddleware, async (req, res) => {
    try {
        const userId = parseInt(req.userId || '0', 10);
 ...
 * Auth: L2 support staff
 */
router.post('/:id/resolve-l2', authMiddleware, async (req, res) => {
    try {
        const userId = parseInt(req.userId || '0', 10);
        const disputeId = parseInt(req.params.id, 10);
        ...
        // Get escalation ID
        const escalationResult = await db.query('SELECT id FROM dispute_escalations WHERE dispute_id = $1 AND level = 2', [disputeId]);
...
router.post('/:id/appeal', authMiddleware, async (req, res) => {
    try {
        ...
        const disputeResult = await db.query('SELECT asker_id, doer_id FROM disputes WHERE id = $1', [disputeId]);
 ...
 */
router.get('/my-assignments/l3', authMiddleware, async (req, res) => {
    try {
        const userId = parseInt(req.userId || '0', 10);
        ...
        const filtered = appeals.filter((a) => a.assigned_to_user_id === userId || a.assigned_to_user_id === null);
...
router.post('/queue/:queueId/assign', authMiddleware, async (req, res) => {
    try {
        ...
        const queueResult = await db.query('SELECT dispute_id FROM support_queue WHERE id = $1', [queueId]);
        if (queueResult.rows.length === 0) {
            return res.status(404).json({ error: 'Queue item not found' });
        }
        const disputeId = queueResult.rows[0].dispute_id;
        // Get escalation ID
        const escalationResult = await db.query('SELECT id FROM dispute_escalations WHERE dispute_id = $1 ORDER BY level DESC LIMIT 1', [disputeId]);
        if (escalationResult.rows.length > 0) {
            await assignDisputeToAgent(escalationResult.rows[0].id, agentId);
        }
        // Update queue status
        await db.query('UPDATE support_queue SET assigned_to_user_id = $1, status = $2 WHERE id = $3', [agentId, 'in_progress', queueId]);
 ...
 * Auth: Support staff
 */
router.post('/queue/:queueId/resolve', authMiddleware, async (req, res) => {
    try {
        const userId = parseInt(req.userId || '0', 10);
        const queueId = parseInt(req.params.queueId, 10);
        // Verify user is support staff
        ...
        // Mark queue item as resolved
        await db.query('UPDATE support_queue SET status = $1, resolved_at = NOW() WHERE id = $2', ['resolved', queueId]);
