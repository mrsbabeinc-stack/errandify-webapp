...
router.post('/subscribe', authMiddleware, async (req, res) => {
    try {
        ...
        await db.query(`INSERT INTO push_subscriptions (user_id, endpoint, auth, p256dh, created_at)
       VALUES ($1, $2, $3, $4, NOW())
...
router.get('/subscriptions', authMiddleware, async (req, res) => {
    try {
        const userId = parseInt(req.userId || '0', 10);
        const result = await db.query('SELECT id, endpoint, created_at FROM push_subscriptions WHERE user_id = $1 ORDER BY created_at DESC', [userId]);
        res.json({
            success: true,
            data: {
