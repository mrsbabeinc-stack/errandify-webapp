...
import { createSubscription, cancelSubscription, } from '../services/subscriptionService.js';
 ...
 */
router.post('/stripe/subscriptions', async (req, res) => {
    const signature = req.headers['stripe-signature'];
    try {
        // Verify webhook signature
        let event;
        try {
            event = stripe.webhooks.constructEvent(req.rawBody || JSON.stringify(req.body), signature, STRIPE_WEBHOOK_SECRET);
        }
        catch (err) {
...
async function handleSubscriptionCreated(subscription) {
    try {
        ...
        await createSubscription(parseInt(companyId), tier, billingType, subscription.id, subscription.customer);
...
async function handleSubscriptionUpdated(subscription) {
    try {
        ...
        await db.query('UPDATE company_subscriptions SET status = ? WHERE stripe_subscription_id = ?', [status, subscription.id]);
...
async function handlePaymentSucceeded(invoice) {
    try {
        ...
        // Log payment success
        const subscription = await db.query('SELECT company_id FROM company_subscriptions WHERE stripe_subscription_id = ?', [invoice.subscription]);
        if (subscription.length > 0) {
            // Create billing history record
            await db.query(`INSERT INTO subscription_billing_history
         (company_id, stripe_invoice_id, amount, status, created_at)
         VALUES (?, ?, ?, 'paid', NOW())`, [subscription[0].company_id, invoice.id, invoice.amount_paid]);
...
async function handlePaymentFailed(invoice) {
    try {
        ...
        if (subscription.length > 0) {
         ...
         VALUES (?, ?, ?, 'failed', ?, NOW())`, [subscription[0].company_id, invoice.id, invoice.amount_due, invoice.last_payment_error?.message]);
