...
router.post('/add-method', authMiddleware, async (req, res) => {
    try {
        const userId = parseInt(req.userId || '0', 10);
...
router.post('/confirm', authMiddleware, async (req, res) => {
    try {
        ...
        if (result.taskId && result.doerId) {
            try {
                await db.query(`UPDATE task_payments SET stripe_intent_id = $1, status = 'completed', confirmed_at = NOW()
           WHERE errand_id = $2`, [intentId, result.taskId]);
            }
            catch (dbError) {
...
router.post('/save-bank-details', authMiddleware, async (req, res) => {
    try {
        const userId = parseInt(req.userId || '0', 10);
...
router.post('/verify-bank', authMiddleware, async (req, res) => {
    try {
        ...
        await db.query('UPDATE users SET bank_verified = true, updated_at = NOW() WHERE id = $1', [userId]);
        res.json({
            success: true,
            data: {
                verified: true,
...
router.post('/link-bank', authMiddleware, async (req, res) => {
    try {
        const userId = parseInt(req.userId || '0', 10);
        ...
        // Get user info
        const userResult = await db.query(`SELECT id, stripe_account_id, email, display_name FROM users WHERE id = $1`, [userId]);
...
router.get('/stripe-account', authMiddleware, async (req, res) => {
    try {
        const userId = parseInt(req.userId || '0', 10);
        const result = await db.query(`SELECT stripe_account_id, bank_verified
       FROM users
       WHERE id = $1`, [userId]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'User not found' });
