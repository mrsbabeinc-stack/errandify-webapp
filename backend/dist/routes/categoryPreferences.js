...
router.get('/:userId/preferences', authMiddleware, async (req, res) => {
    try {
        ...
        const result = await db.query(`SELECT category_can_help, category_need_help FROM users WHERE id = $1`, [userId]);
...
router.post('/:userId/preferences', authMiddleware, async (req, res) => {
    try {
        const { userId } = req.params;
       ...
       WHERE id = $3`, [canHelp || [], needHelp || [], userId]);
        res.json({
            success: true,
...
router.post('/:userId/suggest-categories', authMiddleware, async (req, res) => {
    try {
        const { userId } = req.params;
        const requestingUserId = req.userId ? parseInt(req.userId, 10) : null;
        if (parseInt(userId) !== requestingUserId) {
        ...
        const doerResult = await db.query(`SELECT category, COUNT(*) as count
       FROM errands e
       ...
       ORDER BY count DESC
       LIMIT 3`, [userId]);
        const askerResult = await db.query(`SELECT category, COUNT(*) as count
       FROM errands
