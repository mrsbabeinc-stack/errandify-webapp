...
async function generateSignedUrl(fileType = 'image/jpeg') {
    return {
 ...
 */
router.get('/sign-url', authMiddleware, async (req, res) => {
    try {
...
router.post('/verify', authMiddleware, async (req, res) => {
    try {
        ...
        const assignmentResult = await db.query(`SELECT ea.* FROM errand_assignments ea
       WHERE ea.errand_id = $1 AND ea.doer_id = $2`, [errandId, userId]);
        if (assignmentResult.rows.length === 0) {
            return res.status(403).json({ error: 'You are not assigned to this errand' });
        }
        // Verify errand exists and is in correct status
        const errandResult = await db.query('SELECT id, status FROM errands WHERE id = $1', [errandId]);
        ...
        const photoResult = await db.query(`INSERT INTO task_photos (errand_id, doer_id, photo_url, key, caption, uploaded_at)
       VALUES ($1, $2, $3, $4, $5, NOW())
       RETURNING id, uploaded_at`, [errandId, userId, photoUrl, key, caption || null]);
        res.json({
            success: true,
            data: {
 ...
 */
router.get('/:errandId/photos', authMiddleware, async (req, res) => {
    try {
        const errandId = parseInt(req.params.errandId, 10);
        const userId = parseInt(req.userId || '0', 10);
        // Verify user has access (asker or doer)
        const errandResult = await db.query(`SELECT e.id, e.asker_id FROM errands e
       LEFT JOIN errand_assignments ea ON e.id = ea.errand_id
       WHERE e.id = $1 AND (e.asker_id = $2 OR ea.doer_id = $2)`, [errandId, userId]);
        if (errandResult.rows.length === 0) {
            return res.status(403).json({ error: 'Access denied' });
        }
        // Get photos
        const photosResult = await db.query(`SELECT id, photo_url as url, key, caption, uploaded_at as uploadedAt, doer_id
       FROM task_photos
       WHERE errand_id = $1
       ORDER BY uploaded_at DESC`, [errandId]);
        res.json({
            success: true,
            data: {
...
router.post('/photo', authMiddleware, async (req, res) => {
    try {
        ...
        await db.query(`INSERT INTO evidence_photos (photo_id, user_id, context, mime_type, uploaded_at)
       VALUES ($1, $2, $3, $4, NOW())
       ON CONFLICT DO NOTHING`, [photoId, userId, context || 'dispute_evidence', mimeType]).catch(err => {
