...
router.get('/singpass-authorize', (req, res) => {
    try {
...
router.post('/singpass-callback', async (req, res) => {
    try {
        const { code } = req.body;
        if (!code) {
...
router.post('/signup', async (req, res) => {
    ...
    try {
        ...
        const existing = await db.query('SELECT id FROM users WHERE nric_hash = $1', [hashNric(nric)]);
        ...
        const tempResult = await client.query(`INSERT INTO users (
      ...
      RETURNING id, user_id, display_name, email, mobile, role, criminal_conviction`, [
        ...
        const result = await client.query(`UPDATE users SET formatted_user_id = $1 WHERE id = $2
       RETURNING id, user_id, display_name, email, mobile, role, criminal_conviction, formatted_user_id`, [formattedUserId, newUserId]);
        const user = result.rows[0];
        // AUTO-TRACK REFERRAL: If user signed up via referral code
        if (ref) {
            try {
                // Find referrer by referral code
                const referrerResult = await client.query('SELECT id FROM users WHERE referral_code = $1', [ref]);
             ...
             VALUES ($1, $2, $3, $4)
             ON CONFLICT DO NOTHING`, [referrerId, newUserId, ref, 'joined']);
                    // Award join bonus (50 points) - configurable
                    const joinBonus = 50;
                    await client.query(`INSERT INTO referral_rewards (referrer_id, reward_type, points_amount)
             VALUES ($1, $2, $3)`, [referrerId, 'join', joinBonus]);
                    // Update referrer's errandify_points
                    await client.query(`UPDATE users
             SET errandify_points = errandify_points + $1,
                 updated_at = NOW()
             WHERE id = $2`, [joinBonus, referrerId]);
                    // Log EP transaction for referrer
                    await client.query(`INSERT INTO ep_transactions
             ...
             SELECT
               ...
               NOW()
             FROM users
             WHERE id = $1`, [referrerId, joinBonus, displayName]);
...
router.post('/verify-otp', async (req, res) => {
    try {
        ...
        // Get user
        const result = await db.query('SELECT id, display_name, mobile, role, formatted_user_id FROM users WHERE mobile = $1', [mobile]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'User not found' });
        }
        const user = result.rows[0];
        ...
        // Generate token
        const token = jwt.sign({ userId: user.id, mobile: user.mobile, role: user.role }, config.jwtSecret, { expiresIn: '7d' });
        res.json({
            success: true,
            data: {
                accessToken: token,
...
router.get('/me', authMiddleware, async (req, res) => {
    try {
        const result = await db.query('SELECT id, display_name, mobile, role, language_pref FROM users WHERE id = $1', [req.userId]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'User not found' });
        }
        const user = result.rows[0];
...
router.get('/debug/otp/:mobile', (req, res) => {
...
router.post('/demo-login', async (req, res) => {
    try {
        ...
        let result = await db.query('SELECT id, display_name, mobile, role, chas_card_color, formatted_user_id FROM users WHERE mobile = $1', [demoUser.mobile]);
        let user;
        if (result.rows.length === 0) {
            ...
            const createResult = await db.query(`INSERT INTO users (
        ...
        RETURNING id, display_name, email, mobile, role, chas_card_color, formatted_user_id, gender, bio, certificates`, [
            ...
            const updateResult = await db.query('UPDATE users SET display_name = $1, role = $2 WHERE mobile = $3 RETURNING id, display_name, mobile, role, chas_card_color, formatted_user_id', [demoUser.name, demoUser.defaultRole, demoUser.mobile]);
...
router.post('/singpass/login', (req, res) => {
