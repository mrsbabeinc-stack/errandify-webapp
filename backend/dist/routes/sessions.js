...
router.get('/:errandId/sessions', authMiddleware, async (req, res) => {
    try {
        const errandId = parseInt(req.params.errandId, 10);
        const userId = parseInt(req.userId || '0', 10);
        // Get errand
        const errandResult = await db.query('SELECT * FROM errands WHERE id = $1', [errandId]);
        ...
        if (!isAsker) {
            ...
            const assignmentResult = await db.query('SELECT * FROM errand_assignments WHERE errand_id = $1 AND doer_id = $2', [errandId, userId]);
            if (assignmentResult.rows.length === 0) {
                return res.status(403).json({ error: 'Access denied' });
            }
        }
        // Get sessions
        const sessionsResult = await db.query(`SELECT
       ...
       ORDER BY es.session_number ASC`, [errandId]);
        // Calculate stats
...
router.patch('/:errandId/sessions/:sessionId', authMiddleware, async (req, res) => {
    try {
        const errandId = parseInt(req.params.errandId, 10);
        ...
        // Update session
        const updateResult = await db.query(`UPDATE errand_sessions
         SET status = $1, updated_at = NOW()
         WHERE id = $2
         RETURNING *`, [status, sessionId]);
        if (updateResult.rows.length === 0) {
            return res.status(404).json({ error: 'Session not found' });
        }
        const session = updateResult.rows[0];
        // Get assigned doer for notification
        const assignmentResult = await db.query('SELECT doer_id FROM errand_assignments WHERE session_id = $1', [sessionId]);
        if (assignmentResult.rows.length > 0) {
            const doerId = assignmentResult.rows[0].doer_id;
            // Notify doer
            if (status === 'completed') {
                await createNotification(doerId, 'session_completed', '✅ Session Completed', `Your work on "${errand.title}" (Session ${session.session_number}) has been confirmed!`, null).catch(console.error);
            }
            else if (status === 'cancelled') {
                await createNotification(doerId, 'session_cancelled', '⏸ Session Cancelled', `Session ${session.session_number} of "${errand.title}" has been cancelled.`, null).catch(console.error);
            }
        }
        res.json({
            success: true,
...
router.get('/:errandId/sessions-summary', authMiddleware, async (req, res) => {
    try {
        const errandId = parseInt(req.params.errandId, 10);
