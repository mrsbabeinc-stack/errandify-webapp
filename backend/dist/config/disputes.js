...
// HELPER FUNCTIONS
// ============================================
export function getTimeRemaining(startTime, windowSeconds) {
    const elapsed = (Date.now() - startTime.getTime()) / 1000;
    const remaining = windowSeconds - elapsed;
    return Math.max(0, remaining);
}
export function isHighConfidence(score) {
    return score >= disputeConfig.aiThresholds.autoResolveThreshold;
}
export function isMediumConfidence(score) {
...
export function shouldAutoResolve(score, hasSafetyConcern) {
    if (hasSafetyConcern)
        return false; // Safety always escalates
    return isHighConfidence(score) && disputeConfig.routing.autoResolveHighConfidence;
}
export function shouldEscalateImmediately(hasSafetyConcern, severity) {
    if (!hasSafetyConcern)
        return false;
    return severity === 'critical' || severity === 'high';
}
export function getCancellationPenalty(minutesBeforeStart) {
