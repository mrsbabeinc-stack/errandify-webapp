...
export async function checkAutoPaymentRelease() {
    try {
        ...
        const tasksResult = await db.query(`SELECT e.*, b.doer_id, b.amount FROM errands e
       ...
       ORDER BY e.payment_release_at ASC`, ['completed_unconfirmed']);
...
export async function check24hReminders() {
    try {
        ...
        const tasksResult = await db.query(`SELECT e.*, u.display_name FROM errands e
       JOIN users u ON e.asker_id = u.id
       WHERE e.status = $1
       AND e.payment_release_at > $2
       AND e.payment_release_at < $3
       AND e.reminder_24h_sent = false
       AND e.dispute_status IS NULL`, ['completed_unconfirmed', in23h.toISOString(), in25h.toISOString()]);
        ...
        for (const task of tasksResult.rows) {
            try {
                ...
                await db.query('UPDATE errands SET reminder_24h_sent = true WHERE id = $1', [task.id]);
...
export async function check47hReminders() {
    try {
       ...
       AND e.dispute_status IS NULL`, ['completed_unconfirmed', in46h.toISOString(), in48h.toISOString()]);
        ...
        for (const task of tasksResult.rows) {
            try {
                ...
                await db.query('UPDATE errands SET reminder_47h_sent = true WHERE id = $1', [task.id]);
...
async function releasePaymentForTask(task, reason) {
    ...
    const doerResult = await db.query('SELECT penalty_owed FROM users WHERE id = $1', [task.doer_id]);
    ...
    const releaseResult = await db.query(`INSERT INTO payment_releases (task_id, bid_amount, platform_fee, doer_payout, stripe_transfer_id, released_at, release_reason)
     ...
     RETURNING id, released_at`, [task.id, bidAmount, platformFee, finalPayout, null, reason]);
    // Update task status
    await db.query(`UPDATE errands
     SET status = $1, payment_released_at = NOW()
     WHERE id = $2`, ['completed_confirmed', task.id]);
...
export async function checkEventReminders7Days() {
    try {
        ...
        const result = await db.query(`SELECT e.id, e.title, e.date, e.time, e.location, e.event_link, e.agenda, e.preparation,
       ...
       AND e.status = 'active'`);
        ...
        for (const row of result.rows) {
            try {
                await axios.post(`${process.env.API_URL || 'http://localhost:3000'}/api/email/send-event-reminder-7days`, {
                    email: row.email,
                ...
                await db.query(`UPDATE events SET reminder_7days_sent = true WHERE id = $1`, [row.id]);
            }
            catch (error) {
...
export async function checkEventReminders24Hours() {
    try {
        ...
        const result = await db.query(`SELECT e.id, e.title, e.date, e.time, e.location, e.event_link, e.agenda,
              ea.user_id, u.email, u.display_name
       FROM events e
       JOIN event_attendees ea ON e.id = ea.event_id
       JOIN users u ON ea.user_id = u.id
       ...
       AND e.status = 'active'`);
        ...
        for (const row of result.rows) {
            try {
                await axios.post(`${process.env.API_URL || 'http://localhost:3000'}/api/email/send-event-reminder-24hours`, {
                    email: row.email,
                    eventTitle: row.title,
                    eventDate: row.date,
                    eventTime: row.time,
                ...
                await db.query(`UPDATE events SET reminder_24h_sent = true WHERE id = $1`, [row.id]);
            }
            catch (error) {
...
export async function checkEventReminders1Hour() {
    try {
        ...
        const result = await db.query(`SELECT e.id, e.title, e.time, e.location, e.event_link,
              ea.user_id, u.email, u.display_name
       FROM events e
       JOIN event_attendees ea ON e.id = ea.event_id
       JOIN users u ON ea.user_id = u.id
       ...
       AND e.status = 'active'`);
        ...
        for (const row of result.rows) {
            try {
                await axios.post(`${process.env.API_URL || 'http://localhost:3000'}/api/email/send-event-reminder-1hour`, {
                    email: row.email,
                    eventTitle: row.title,
                    eventTime: row.time,
                    eventLocation: row.location,
                ...
                await db.query(`UPDATE events SET reminder_1h_sent = true WHERE id = $1`, [row.id]);
            }
            catch (error) {
...
export async function checkEventRemindersDayOf() {
    try {
       ...
       AND e.status = 'active'`);
        ...
        for (const row of result.rows) {
            try {
                await axios.post(`${process.env.API_URL || 'http://localhost:3000'}/api/email/send-event-reminder-dayof`, {
                    email: row.email,
                    eventTitle: row.title,
                    eventDate: row.date,
                    eventTime: row.time,
                ...
                await db.query(`UPDATE events SET reminder_dayof_sent = true WHERE id = $1`, [row.id]);
            }
            catch (error) {
