...
function setupSocketHandlers() {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('send-message', async (data) => {
           ...
           RETURNING id, created_at`, [conversationId, userId, message]);
        ...
        socket.on('mark-read', async (messageIds) => {
            try {
                ...
                const result = await db.query(`SELECT DISTINCT conversation_id FROM chat_messages WHERE id = ANY($1)`, [messageIds]);
        ...
        socket.on('request-messages-since', async (data) => {
            try {
 ...
 */
export function broadcastToConversation(conversationId, eventName, data) {
    if (!io)
        return;
    const roomName = `conversation_${conversationId}`;
 ...
 */
export function getUsersInConversation(conversationId) {
    const roomName = `conversation_${conversationId}`;
