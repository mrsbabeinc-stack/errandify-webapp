...
export async function validatePaymentRelease(errandId, requesterId) {
    try {
        const errand = await db.query(`SELECT id, asker_id, doer_id, status, completed_at, asker_confirmed FROM errands WHERE id = $1`, [errandId]);
...
export async function releasePayment(errandId, requesterId, askerConfirmation) {
    ...
    try {
        ...
        const result = await db.query(`UPDATE errands SET status = 'payment_released', asker_confirmed = true, payment_released_at = NOW() WHERE id = $1 RETURNING id, status, payment_released_at`, [errandId]);
...
export async function getPaymentStatus(errandId) {
    try {
        const result = await db.query(`SELECT id, status, completed_at, payment_released_at, asker_confirmed, asker_id, doer_id FROM errands WHERE id = $1`, [errandId]);
        if (result.rows.length === 0)
            return null;
