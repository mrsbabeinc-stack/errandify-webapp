...
export class LeaveService {
    ...
    static async isStaffOnLeave(staffId, checkDate) {
        try {
            const result = await db.query(`SELECT id FROM leave_requests
         WHERE staff_id = $1
         AND status = 'approved'
         AND $2::DATE BETWEEN start_date AND end_date`, [staffId, checkDate.toISOString().split('T')[0]]);
    ...
    static async isStaffOnLeaveRange(staffId, startDate, endDate) {
        try {
            const result = await db.query(`SELECT id, leave_type, start_date, end_date, period
         FROM leave_requests
         WHERE staff_id = $1
         AND status = 'approved'
         AND (
    ...
    static async getLeaveBalance(staffId, year = new Date().getFullYear()) {
        try {
         ...
         WHERE staff_id = $1 AND year = $2`, [staffId, year]);
            return result.rows;
        }
        catch (error) {
    ...
    static async deductLeaveDays(staffId, leaveType, days, year = new Date().getFullYear()) {
        try {
            await db.query(`UPDATE leave_entitlements
         SET used_days = used_days + $1,
             remaining_days = entitlement_days - (used_days + $1),
             last_updated = NOW()
         WHERE staff_id = $2 AND leave_type = $3 AND year = $4`, [days, staffId, leaveType, year]);
        }
        catch (error) {
    ...
    static async restoreLeaveDays(staffId, leaveType, days, year = new Date().getFullYear()) {
        try {
            await db.query(`UPDATE leave_entitlements
