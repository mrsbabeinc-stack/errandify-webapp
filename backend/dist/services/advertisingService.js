...
export const advertisingService = {
    async approveCampaign(campaignId, _adminId, adminNotes) {
        ...
        try {
            ...
            if (availableCents < budgetInCents) {
                throw new Error(`Insufficient ad credits. Available: SGD $${(availableCents / 100).toFixed(2)}, Required: SGD $${campaign.budget.toFixed(2)}. Please wait for next month's allocation or upgrade your plan.`);
            }
         ...
         VALUES ($1, $2, $3, 'campaign_approved', NOW())`, [campaign.company_id, campaignId, budgetInCents]);
    ...
    async rejectCampaign(campaignId, _adminId, rejectionReason) {
        const campaign = await campaignModel.getById(campaignId);
        if (!campaign)
            throw new Error('Campaign not found');
        ...
        if (campaign.status === 'approved') {
            ...
            // Log refund
            await db.query(`INSERT INTO ad_credit_usage_log (company_id, campaign_id, amount, action, created_at)
         VALUES ($1, $2, $3, 'campaign_rejected_refund', NOW())`, [campaign.company_id, campaignId, budgetInCents]);
        }
    ...
    async pauseCampaign(campaignId) {
        ...
        if (campaign.status !== 'live')
            throw new Error('Only live campaigns can be paused');
