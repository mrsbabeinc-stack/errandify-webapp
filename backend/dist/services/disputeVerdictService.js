...
import { validateDisputeVerdict, generateAuditReport } from './disputeVerdictValidator.js';
/**
 * Use Qwen AI to craft a detailed verdict explanation
 * Takes the dispute details and generates human-friendly reasoning
 */
export async function craftDisputeVerdict(disputeId, userRole, claimantStatement, defendantStatement, evidence, aiRecommendation) {
    try {
...
Return JSON: {
  ...
  "logic": "Step-by-step analysis with bullet points or numbered list"
        ...
        const response = await QwenAI.call([{ role: 'user', content: prompt }], { temperature: 0.5, maxTokens: 1000 });
        let verdictText = {
        ...
        const validation = await validateDisputeVerdict(disputeId, claimantStatement, defendantStatement, verdictData, evidence);
        if (!validation.isValid) {
            ...
            if (validation.safetyFlags.length > 0) {
                ...
                await db.query(`INSERT INTO dispute_compliance_queue
           (dispute_id, queue_reason, severity, description)
           VALUES ($1, $2, $3, $4)`, [disputeId, 'safety_flag', 'critical', validation.safetyFlags[0].description]);
            }
            // Log all issues
            for (const issue of validation.issues) {
                if (issue.severity === 'critical' || issue.severity === 'high') {
                    await db.query(`INSERT INTO dispute_compliance_queue
             (dispute_id, queue_reason, severity, description)
             VALUES ($1, $2, $3, $4)`, [disputeId, issue.type, issue.severity, `${issue.message} - ${issue.recommendation}`]);
                }
            }
 ...
 */
export function generateDisputeNotification(verdict, userRole) {
 ...
 */
export async function saveDisputeVerdict(disputeId, verdict, decidedBy = 'ai') {
    try {
        // First, get the errand details for amounts
        const dispute = await db.query(`SELECT errand_id FROM disputes WHERE id = $1`, [disputeId]);
        if (dispute.rows.length === 0)
            return false;
        const errandId = dispute.rows[0].errand_id;
        // Get errand to calculate actual amounts
        const errand = await db.query(`SELECT budget FROM errands WHERE id = $1`, [errandId]);
        ...
        await db.query(`INSERT INTO dispute_decisions
