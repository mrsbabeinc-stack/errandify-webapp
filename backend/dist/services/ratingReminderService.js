 ...
 */
export async function scheduleRatingReminder(errandId, doerId) {
    try {
        // Get errand details
        const errandResult = await db.query(`SELECT e.id, e.asker_id
       FROM errands e
       WHERE e.id = $1`, [errandId]);
        ...
        await db.query(`INSERT INTO pending_reminders (errand_id, doer_id, asker_id, reminder_type, scheduled_for, created_at)
       VALUES ($1, $2, $3, $4, $5, NOW())
       ON CONFLICT (errand_id, doer_id, reminder_type) DO NOTHING`, [errandId, doerId, askerId, 'rating_reminder_online', fallbackEmailTime]);
 ...
 */
export async function sendPendingRatingReminders() {
    try {
        ...
        const remindersResult = await db.query(`SELECT pr.id, pr.errand_id, pr.doer_id, pr.asker_id,
       ...
       LIMIT 100`, ['rating_reminder_online']);
        ...
        for (const reminder of remindersResult.rows) {
            try {
                ...
                const ratingCheckResult = await db.query(`SELECT id FROM ratings WHERE errand_id = $1 AND rater_id = $2`, [errandId, doerId]);
