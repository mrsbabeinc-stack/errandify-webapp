 ...
 */
export async function screenErrandForSafety(errandId, userId, title, description) {
    try {
        ...
        for (const [category, keywords] of Object.entries(TRAFFICKING_KEYWORDS)) {
            for (const keyword of keywords) {
                if (fullText.includes(keyword)) {
                    ...
                    const weight = SEVERITY_WEIGHTS[category] || 0.5;
 ...
 */
export async function storeSafetyFlag(flag) {
    try {
        await db.query(`INSERT INTO safety_flags
 ...
 */
export async function getUserSafetyFlags(userId) {
    try {
        const result = await db.query(`SELECT * FROM safety_flags
       WHERE user_id = $1
       ORDER BY reported_at DESC`, [userId]);
 ...
 */
export async function resolveSafetyFlag(flagId, resolutionType, notes) {
    try {
        await db.query(`UPDATE safety_flags
       SET resolved_at = NOW(), resolution_type = $1, notes = $2
       WHERE id = $3`, [resolutionType, notes, flagId]);
    }
    catch (error) {
 ...
 */
export async function checkUserBehaviorPattern(userId) {
    try {
        // Get user's recent errand postings
        const result = await db.query(`SELECT description, title, budget
       FROM errands
       WHERE asker_id = $1
       ...
       ORDER BY created_at DESC`, [userId]);
        ...
        // Analyze patterns
        let redFlagCount = 0;
        const allText = errands.map((e) => `${e.title} ${e.description}`).join(' ').toLowerCase();
        ...
        const averagePay = errands.reduce((sum, e) => sum + (e.budget || 0), 0) / errands.length;
 ...
 */
export async function submitVulnerabilityReport(reportType, description, relatedErrandId, relatedUserId, contactPhone, contactEmail, severity) {
    try {
        const result = await db.query(`INSERT INTO vulnerability_reports
       (report_type, description, related_errand_id, related_user_id, contact_phone, contact_email, severity, status, created_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, 'new', NOW())
       RETURNING id`, [reportType, description, relatedErrandId || null, relatedUserId || null, contactPhone || null, contactEmail || null, severity || 'medium']);
