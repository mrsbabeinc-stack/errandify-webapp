...
export class OperationHoursService {
    ...
    static async getCompanyHours(companyId) {
        try {
    ...
    static async isWithinOperatingHours(companyId, checkDateTime) {
        try {
    ...
    static async updateCompanyHours(companyId, hours) {
        try {
            const updates = [];
    ...
    static async getStaffAvailability(staffId, date) {
        try {
            const result = await db.query(`SELECT status, reason, notes
         FROM staff_availability
         WHERE staff_id = $1 AND availability_date = $2::DATE`, [staffId, date.toISOString().split('T')[0]]);
    ...
    static async setStaffAvailability(companyId, staffId, date, status, reason) {
        try {
            const result = await db.query(`INSERT INTO staff_availability (company_id, staff_id, availability_date, status, reason)
         VALUES ($1, $2, $3::DATE, $4, $5)
         ON CONFLICT (company_id, staff_id, availability_date)
         DO UPDATE SET status = $4, reason = $5
         RETURNING *`, [companyId, staffId, date.toISOString().split('T')[0], status, reason || '']);
            return result.rows[0];
        }
        catch (error) {
