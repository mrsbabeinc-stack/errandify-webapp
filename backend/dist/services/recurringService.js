...
export const generateRecurringInstances = async (parentErrandId, startDate, config) => {
    const instances = [];
    // Calculate the number of occurrences
    const occurrences = config.occurrences || 1;
    // Get parent errand details
    const parentResult = await db.query(`SELECT id, asker_id, title, description, category, location, postal_code, budget, is_recurring, recurring_schedule, formatted_id
     FROM errands WHERE id = $1`, [parentErrandId]);
    ...
    for (let i = 1; i <= occurrences; i++) {
        ...
        const instanceResult = await db.query(`INSERT INTO errands (
        asker_id, title, description, category, location, postal_code, budget, deadline, status, errand_id
      ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, 'open', $9)
       RETURNING id`, [parent.asker_id, parent.title, parent.description, parent.category, parent.location, parent.postal_code, parent.budget, scheduledDate, errandId]);
        const instanceId = instanceResult.rows[0].id;
        instances.push(instanceId);
        // Link to parent via recurring_sessions
        await db.query(`INSERT INTO recurring_sessions (parent_errand_id, instance_number, errand_id, scheduled_date)
       VALUES ($1, $2, $3, $4)`, [parentErrandId, i, instanceId, scheduledDate]);
...
const calculateNextDate = (baseDate, repeatEvery, repeatUnit, offsetCount) => {
...
export const getRecurringInstances = async (parentErrandId) => {
    const result = await db.query(`SELECT rs.id, rs.instance_number, rs.errand_id, rs.scheduled_date, e.title, e.status, e.budget
     FROM recurring_sessions rs
     JOIN errands e ON rs.errand_id = e.id
     WHERE rs.parent_errand_id = $1
     ORDER BY rs.instance_number ASC`, [parentErrandId]);
    return result.rows;
};
// Get parent errand of a recurring instance
export const getParentErrand = async (instanceErrandId) => {
    const result = await db.query(`SELECT rs.parent_errand_id, rs.instance_number,
            (SELECT COUNT(*) FROM recurring_sessions WHERE parent_errand_id = rs.parent_errand_id) as total_instances
     FROM recurring_sessions rs
     WHERE rs.errand_id = $1`, [instanceErrandId]);
...
export const isRecurringInstance = async (errandId) => {
    const result = await db.query('SELECT id FROM recurring_sessions WHERE errand_id = $1 LIMIT 1', [errandId]);
