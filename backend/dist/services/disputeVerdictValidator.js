 ...
 */
export async function validateDisputeVerdict(disputeId, claimantStatement, defendantStatement, proposedVerdict, evidence) {
    const issues = [];
    const safetyFlags = [];
    const legalRisks = [];
    ...
    try {
        ...
        const biasAnalysis = await detectBias(claimantStatement, defendantStatement, proposedVerdict.reasoning, evidence);
        ...
        const fairnessAnalysis = await validateFairness(claimantStatement, defendantStatement, proposedVerdict, evidence);
        ...
        const legalAnalysis = await validateLegalCompliance(proposedVerdict, claimantStatement, defendantStatement, evidence);
 ...
 */
async function analyzeSafetyRisks(claimantStatement, defendantStatement) {
    ...
    for (const keyword of criticalKeywords) {
        if (textLower.includes(keyword)) {
            let flagType = 'fraud';
            if (['threatened', 'coerce', 'blackmail', 'extort'].includes(keyword))
                flagType = 'coercion';
            if (['hit', 'hurt', 'injured', 'attack', 'violence'].includes(keyword))
                flagType = 'injury';
            if (['abuse', 'harass', 'stalk', 'assault'].includes(keyword))
                flagType = 'abuse';
            if (['trafficking', 'exploitation', 'slavery'].includes(keyword))
                flagType = 'abuse';
            flags.push({
                type: flagType,
                severity: 'critical',
 ...
 */
async function detectBias(claimantStatement, defendantStatement, reasoning, evidence) {
 ...
 */
async function validateFairness(claimantStatement, defendantStatement, verdict, evidence) {
 ...
 */
async function validateLegalCompliance(verdict, claimantStatement, defendantStatement, evidence) {
    const risks = [];
 ...
 */
async function validateConsistency(verdict) {
...
Return JSON: { "consistent": true/false }`;
    try {
        const response = await QwenAI.call([{ role: 'user', content: prompt }], { temperature: 0.2, maxTokens: 200 });
        let analysis = { consistent: true };
        try {
            analysis = JSON.parse(response);
        }
        catch (e) {
 ...
 */
export async function logVerdictAuditTrail(disputeId, userId, verdict, validation) {
    try {
        await db.query(`INSERT INTO dispute_audit_trail
 ...
 */
export function generateAuditReport(validation) {
    const report = `
