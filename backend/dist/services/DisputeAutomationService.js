...
export class DisputeAutomationService {
    ...
    // Reminder 1 at T+24h
    static async handleReminder1() {
        const targetTime = new Date(Date.now() - 24 * 60 * 60 * 1000); // T+24h ago
        const result = await db.query(`SELECT d.id, d.response_deadline, u.email, u.name
       FROM disputes d
       JOIN users u ON d.defendant_user_id = u.id
       WHERE d.status = 'OPEN'
       AND d.first_reminder_sent_at IS NULL
       AND d.created_at <= $1`, [targetTime]);
        for (const row of result.rows) {
            await db.query(`UPDATE disputes SET first_reminder_sent_at = NOW() WHERE id = $1`, [row.id]);
            // TODO: Send email
            console.log(`[Reminder1] Sent to ${row.email} for dispute ${row.id}`);
        }
    }
    // Reminder 2 at T+36h (URGENT)
    static async handleReminder2() {
        const targetTime = new Date(Date.now() - 36 * 60 * 60 * 1000); // T+36h ago
        const result = await db.query(`SELECT d.id, d.response_deadline, u.email, u.name, u.phone
       FROM disputes d
       JOIN users u ON d.defendant_user_id = u.id
       WHERE d.status = 'OPEN'
       ...
       AND d.created_at <= $1`, [targetTime]);
        for (const row of result.rows) {
            await db.query(`UPDATE disputes SET second_reminder_sent_at = NOW() WHERE id = $1`, [row.id]);
    ...
    static async handleAutoResolution() {
        const targetTime = new Date(Date.now() - 48 * 60 * 60 * 1000); // T+48h ago
        const result = await db.query(`SELECT d.id, d.amount, d.errand_id, d.defendant_user_id, d.asker_id
       FROM disputes d
       WHERE d.status IN ('OPEN', 'PENDING_RESPONSE')
       AND d.created_at <= $1
       AND d.verdict_issued_at IS NULL`, [targetTime]);
        for (const row of result.rows) {
            await this.executeAutoResolution(row.id, row.amount);
        }
    }
    static async executeAutoResolution(disputeId, totalAmount) {
        const now = new Date();
        // Get evidence counts
        const evidenceResult = await db.query(`SELECT submitted_by, COUNT(*) as count
       FROM dispute_evidence
       WHERE dispute_id = $1
       GROUP BY submitted_by`, [disputeId]);
        const doerCount = evidenceResult.rows.find((r) => r.submitted_by === 'doer')?.count || 0;
        const companyCount = evidenceResult.rows.find((r) => r.submitted_by === 'company')?.count || 0;
       ...
       WHERE id = $7`, [
        ...
        console.log(`[AutoResolution] Dispute ${disputeId}: ${decision} (Doer: $${doerAmount}, Company: $${companyAmount})`);
    }
    ...
    static async checkAndResolveAppeals() {
        ...
        const result = await db.query(`SELECT d.id, d.verdict_issued_at, d.appeal_submitted_at
       FROM disputes d
       WHERE d.status = 'APPEALED'
       AND d.appeal_reviewed_at IS NULL
       AND d.verdict_issued_at <= $1`, [appealDeadline]);
    ...
    static async processPaymentSettlements() {
        const result = await db.query(`SELECT d.id, d.verdict_doer_amount, d.verdict_company_amount, d.errand_id, d.closed_at
       FROM disputes d
