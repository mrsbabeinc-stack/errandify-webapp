...
export async function getUserReferralCode(userId) {
    try {
        ...
        await db.query('UPDATE users SET referral_code = $1 WHERE id = $2', [newCode, userId]);
        return newCode;
    }
    catch (error) {
 ...
 */
export async function trackReferralJoin(referrerId, referredUserId, referralCode) {
    const client = await db.getClient();
    try {
        await client.query('BEGIN');
        // Check if already tracked
        const existing = await client.query('SELECT id FROM referral_tracking WHERE referrer_id = $1 AND referred_user_id = $2', [referrerId, referredUserId]);
       ...
       VALUES ($1, $2, $3, $4)`, [referrerId, referredUserId, referralCode, 'joined']);
 ...
 */
export async function trackReferralFirstJob(referrerId, referredUserId) {
    const client = await db.getClient();
    try {
        await client.query('BEGIN');
        // Check if tracking exists
        const tracking = await client.query(`SELECT id FROM referral_tracking
       WHERE referrer_id = $1 AND referred_user_id = $2`, [referrerId, referredUserId]);
        if (tracking.rows.length === 0) {
            await client.query('COMMIT');
            return { success: false, points_awarded: 0 };
        }
        // Check if first job bonus already awarded
        const bonus = await client.query(`SELECT id FROM referral_rewards
       ...
       AND referrer_id IN (
        ...
        await client.query(`UPDATE referral_tracking
...
export async function getReferralStats(userId) {
    try {
      ...
      WHERE referrer_id = $1`, [userId]);
        ...
        const pointsResult = await db.query(`SELECT
        reward_type,
        SUM(points_amount) as total_points
      FROM referral_rewards
      WHERE referrer_id = $1
      GROUP BY reward_type`, [userId]);
        ...
        for (const row of pointsResult.rows) {
            const key = `${row.reward_type}_bonus`;
 ...
 */
export async function getReferralRewards(userId, limit = 50) {
    try {
        const result = await db.query(`SELECT id, referrer_id, reward_type, points_amount, awarded_at,
        CASE
...
async function awardReferralPoints(client, referrerId, points, rewardType) {
    ...
    await client.query(`INSERT INTO ep_transactions
     ...
     SELECT
       ...
       NOW()
     FROM users
     WHERE id = $1`, [referrerId, rewardType, points]);
}
/**
