...
class DisputeService {
    ...
    static async submitResponse(input, evidenceFiles) {
    ...
    static async submitEvidence(disputeId, userId, userType, evidenceFiles) {
    ...
    static async getEvidenceByParty(disputeId, party) {
    ...
    static async getAllEvidence(disputeId) {
        const evidence = await DisputeEvidence.findAll({
            where: { disputeId },
            order: [['uploadedAt', 'DESC']],
        });
    ...
    static async requestExtension(disputeId, reason, requestedBy) {
        const dispute = await Dispute.findByPk(disputeId);
        if (!dispute)
            throw new Error('Dispute not found');
    ...
    static async approveExtension(disputeId, approvedBy) {
        const dispute = await Dispute.findByPk(disputeId);
        if (!dispute)
            throw new Error('Dispute not found');
        if (!dispute.extensionRequested) {
    ...
    static async denyExtension(disputeId, deniedBy, reason) {
        const dispute = await Dispute.findByPk(disputeId);
        if (!dispute)
            throw new Error('Dispute not found');
        await dispute.update({
    ...
    static async executeAutoResolution(disputeId) {
        const dispute = await Dispute.findByPk(disputeId);
        if (!dispute)
            return;
    ...
    static async issueVerdict(disputeId, decision, doerAmount, companyAmount, reasoning, issuedBy) {
        const dispute = await Dispute.findByPk(disputeId);
        if (!dispute)
            throw new Error('Dispute not found');
        await dispute.update({
    ...
    static async submitAppeal(disputeId, reason, userId, evidenceFiles) {
        const dispute = await Dispute.findByPk(disputeId);
        if (!dispute)
            throw new Error('Dispute not found');
    ...
    static async resolveAppeal(disputeId, decision, reasoning, newDoerAmount, newCompanyAmount, resolvedBy) {
        const dispute = await Dispute.findByPk(disputeId);
        if (!dispute)
            throw new Error('Dispute not found');
        if (!dispute.appealed) {
