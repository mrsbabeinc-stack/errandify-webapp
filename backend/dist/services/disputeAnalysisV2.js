 ...
 */
export async function analyzeSafety(description) {
 ...
 */
export function scoreEvidence(hasGps, photoCount, hasChat, hasWaitTime, descriptionWordCount, disputeType) {
    const requirements = disputeConfig.evidenceRequirements[disputeType];
    if (!requirements) {
 ...
 */
export async function detectPatterns(doerId, askerId) {
    try {
        // Get doer dispute history
        const doerDisputes = await db.query(`SELECT COUNT(*) as count FROM disputes
       WHERE filed_by_user_id = $1 OR user_id = $1
       AND created_at > NOW() - INTERVAL '30 days'`, [doerId]);
        ...
        const askerDisputes = await db.query(`SELECT COUNT(*) as count FROM disputes
       WHERE filed_by_user_id = $1
       AND status = 'resolved'
       AND created_at > NOW() - INTERVAL '30 days'`, [askerId]);
        const askerDefenseCount = parseInt(askerDisputes.rows[0]?.count || '0');
        // Get ratings
        const doerRatings = await db.query(`SELECT AVG(CAST(rating as DECIMAL)) as avg_rating FROM ratings
       WHERE rated_user_id = $1`, [doerId]);
        const doerRating = parseFloat(doerRatings.rows[0]?.avg_rating || '4.5');
        const askerRatings = await db.query(`SELECT AVG(CAST(rating as DECIMAL)) as avg_rating FROM ratings
       WHERE rated_user_id = $1`, [askerId]);
        const askerRating = parseFloat(askerRatings.rows[0]?.avg_rating || '4.5');
        // Get job counts
        const doerJobs = await db.query(`SELECT COUNT(*) as count FROM errands WHERE doer_id = $1`, [doerId]);
        const doerJobCount = parseInt(doerJobs.rows[0]?.count || '0');
        const askerJobs = await db.query(`SELECT COUNT(*) as count FROM errands WHERE asker_id = $1`, [askerId]);
 ...
 */
export function calculateConfidenceScore(evidenceScore, safetyFactor, // 0 = no concern, -0.2 = medium concern, -0.3 = high concern
patternRisk, // 0 = green, 0.1 = yellow, 0.3 = red
plausibility // 0-1
) {
 ...
 */
export async function analyzeDispute(disputeId, filedByUserId, otherPartyId, description, evidence, disputeType) {
    try {
        // Safety analysis
        const safetyAnalysis = await analyzeSafety(description);
        // Evidence scoring
        const wordCount = description.split(/\s+/).length;
        const evidenceScore = scoreEvidence(evidence.hasGps, evidence.photoCount, evidence.hasChat, evidence.hasWaitTime, wordCount, disputeType);
        // Pattern detection
        ...
        // Pattern risk
        let patternRisk = 0;
        if (patterns.patternFlag === 'RED')
            patternRisk = 0.3;
        else if (patterns.patternFlag === 'YELLOW')
            patternRisk = 0.1;
        // Calculate confidence
        const confidenceScore = calculateConfidenceScore(evidenceScore, safetyFactor, patternRisk, plausibilityScore);
        // Determine if auto-resolvable
        const canAutoResolve = confidenceScore >= disputeConfig.aiThresholds.autoResolveThreshold &&
            !safetyAnalysis.hasConcern;
        const escalateImmediately = safetyAnalysis.severity === 'critical' || safetyAnalysis.severity === 'high';
        const humanReviewNeeded = !canAutoResolve || escalateImmediately;
        // Recommend decision
        let recommendedDecision = 'partial_payment';
    ...
    `.trim();
        const result = {
            ...
            safetySeverity: (safetyAnalysis.severity || 'low'),
 ...
 */
export async function saveAnalysis(disputeId, analysis) {
    try {
        await db.query(`INSERT INTO dispute_ai_analysis (
        dispute_id,
        safety_concern,
        safety_severity,
        safety_concern_type,
