...
export async function analyzeDisputeSafety(description) {
    try {
        ...
        const recommendation = severity === 'critical' || severity === 'high' ? 'escalate_immediately' : 'monitor';
        return {
            hasConcern: true,
            concernType,
            severity,
...
export async function autoResolveDispute(disputeId) {
    try {
        const dispute = await db.query(`SELECT id, errand_id, dispute_type, description FROM disputes WHERE id = $1`, [disputeId]);
        if (dispute.rows.length === 0) {
            return { resolved: false, reason: 'Dispute not found' };
        }
        const d = dispute.rows[0];
        // Rule 1: No chat usage = auto-refund to asker
        const chatCount = await db.query(`SELECT COUNT(*) as count FROM messages
       WHERE related_errand_id = $1`, [d.errand_id]);
        if (chatCount.rows[0].count === 0) {
            // Refund asker
            await db.query(`UPDATE errands SET status = 'dispute_resolved_auto', resolution = 'refunded' WHERE id = $1`, [d.errand_id]);
        ...
        const errand = await db.query(`SELECT status, payment_released_at FROM errands WHERE id = $1`, [d.errand_id]);
        if (!errand.rows[0].payment_released_at) {
            await db.query(`UPDATE errands SET status = 'dispute_resolved_auto', resolution = 'pending_payment' WHERE id = $1`, [d.errand_id]);
...
export async function analyzeDisputeWithAI(disputeId) {
    try {
        const dispute = await db.query(`SELECT id, errand_id, dispute_type, filed_by, description, evidence, defendant_response, defendant_response_evidence, response_status FROM disputes WHERE id = $1`, [disputeId]);
        if (dispute.rows.length === 0) {
            return {
                confidenceLevel: 0,
...
Return JSON: {
  ...
  "evidenceStrength": 0.0-1.0
        ...
        const response = await QwenAI.call([{ role: 'user', content: prompt }], { temperature: 0.3, maxTokens: 500 });
        let analysis = {
            confidence: 0.5,
            resolution: 'escalate',
...
export async function escalateDispute(disputeId, notes, priority = 'normal') {
    try {
        const result = await db.query(`UPDATE disputes
       SET status = 'escalated', priority = $1, escalation_notes = $2, escalated_at = NOW()
       WHERE id = $3
       RETURNING id, status, priority`, [priority, notes, disputeId]);
...
export async function createDispute(params) {
    try {
        ...
        const result = await db.query(`INSERT INTO disputes (errand_id, filed_by_user_id, dispute_type, description, evidence, status, priority)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       RETURNING id, status, created_at`, [params.errandId, params.filedByUserId, params.type, params.description, params.evidence || null, status, priority]);
        ...
        if (status === 'level_1' && tierClassification.tier === 'auto') {
            ...
            if (!autoResolve.resolved) {
                ...
                await db.query(`UPDATE disputes SET status = 'level_2' WHERE id = $1`, [disputeId]);
            }
        }
...
export async function classifyDisputeTier(disputeId) {
    try {
        const dispute = await db.query(`SELECT id, errand_id, filed_by_user_id, dispute_type, description, evidence FROM disputes WHERE id = $1`, [disputeId]);
        if (dispute.rows.length === 0) {
        ...
        if (evidence.gpsLocation)
            claimantScore += 0.4;
        // Multiple photos (strong visual evidence)
        if (evidence.photos && evidence.photos.length >= 2)
            claimantScore += 0.3;
        else if (evidence.photos && evidence.photos.length === 1)
            claimantScore += 0.15;
        // Wait time (shows effort/attempt)
        if (evidence.waitTime && evidence.waitTime >= 20)
            claimantScore += 0.15;
        else if (evidence.waitTime)
            claimantScore += 0.05;
        ...
        const responseDeadline = defendantNeedsResponse
            ...
            : undefined;
        await db.query(`INSERT INTO dispute_tier_classification
       ...
       ON CONFLICT (dispute_id) DO UPDATE SET
        ...
        if (defendantNeedsResponse && responseDeadline) {
            const errand = await db.query(`SELECT doer_id, asker_id FROM errands WHERE id = $1`, [d.errand_id]);
            if (errand.rows.length > 0) {
                ...
                await db.query(`INSERT INTO dispute_defense_requests (dispute_id, defendant_user_id, request_reason, deadline, notified_at)
           VALUES ($1, $2, $3, $4, NOW())
           ON CONFLICT (dispute_id) DO NOTHING`, [disputeId, defendantId, tier === 'statement_only' ? 'asker_has_strong_evidence' : 'evidence_unclear', responseDeadline]);
           ...
           WHERE id = $4`, [defendantId, responseDeadline, tier, disputeId]);
            }
        }
...
export async function holdPayment(errandId, reason) {
    try {
        await db.query(`UPDATE errands SET payment_held = true, payment_held_reason = $1, payment_held_at = NOW() WHERE id = $2`, [reason, errandId]);
...
export async function releaseHeldPayment(errandId) {
    try {
        await db.query(`UPDATE errands SET payment_held = false, payment_held_reason = NULL WHERE id = $1`, [errandId]);
...
export async function getDisputeStatus(disputeId) {
    try {
        const result = await db.query(`SELECT id, errand_id, filed_by_user_id, dispute_type, description, status, priority, created_at, resolved_at
