 ...
 */
export async function checkMilestones(companyId) {
    const subscription = await getCompanySubscription(companyId);
    if (!subscription || subscription.status !== 'active') {
        return; // No active subscription
    }
    // Get total tasks posted
    const result = await db.query('SELECT COUNT(*) as total FROM tasks WHERE company_id = ? AND status = "posted"', [companyId]);
    ...
    for (const milestone of milestones) {
        if (totalTasks >= milestone.tasks) {
            await awardMilestone(companyId, `tasks_posted_${milestone.tasks}`, milestone.bonus);
        }
    }
}
/**
 ...
 */
async function awardMilestone(companyId, milestoneType, bonusAmount) {
    // Check if already awarded
    const existing = await db.query('SELECT id FROM subscription_milestones WHERE company_id = ? AND milestone_type = ?', [companyId, milestoneType]);
    if (existing.length > 0) {
        return; // Already awarded
    }
    // Create milestone record
    await db.query(`INSERT INTO subscription_milestones
     (company_id, milestone_type, completed_at, bonus_amount, bonus_applied, created_at)
     VALUES (?, ?, NOW(), ?, FALSE, NOW())`, [companyId, milestoneType, bonusAmount]);
    // Add bonus credits
    await addBonusCredits(companyId, bonusAmount, `Milestone: ${milestoneType}`);
    // Mark as applied
    await db.query('UPDATE subscription_milestones SET bonus_applied = TRUE WHERE company_id = ? AND milestone_type = ?', [companyId, milestoneType]);
 ...
 */
export async function getMilestones(companyId) {
    const result = await db.query('SELECT * FROM subscription_milestones WHERE company_id = ? ORDER BY completed_at DESC', [companyId]);
    return result;
}
/**
 * Get next milestone for company
 */
export async function getNextMilestone(companyId) {
    const subscription = await getCompanySubscription(companyId);
    if (!subscription || subscription.status !== 'active') {
        return null;
    }
    ...
    for (const milestone of milestones) {
        const completed = await db.query('SELECT id FROM subscription_milestones WHERE company_id = ? AND milestone_type = ?', [companyId, `tasks_posted_${milestone.tasks}`]);
 ...
 */
export async function getMilestoneProgress(companyId) {
    const subscription = await getCompanySubscription(companyId);
    if (!subscription || subscription.status !== 'active') {
        return {
            current_tasks: 0,
