 ...
 */
export function generateAutoResolveMessageToDoer(context) {
    const decision = context.decision === 'full_payment'
        ? 'APPROVED ✅'
        : context.decision === 'partial_payment'
            ? 'PARTIAL PAYMENT 💰'
            : 'REFUND ISSUED 💵';
    const amountText = context.decision === 'full_payment'
 ...
 */
export function generateAutoResolveMessageToAsker(context) {
    const decision = context.decision === 'full_payment'
        ? 'Claim Approved ✓'
        : context.decision === 'partial_payment'
            ? 'Partial Refund 💰'
            : 'Full Refund 💵';
    const impactText = context.decision === 'full_payment'
 ...
 */
export function generateHumanReviewMessageToDoer(context) {
    return `
 ...
 */
export function generateHumanReviewMessageToAsker(context) {
    return `
 ...
 */
export function generateEscalationNotice(context) {
    return `
...
// HELPER FUNCTIONS
// ============================================
function getDisputeExplanation(analysis, disputeType, isDoer) {
    if (isDoer) {
...
function getEvidenceSummary(analysis) {
    const evidence = [];
    if (analysis.hasGpsData)
        evidence.push('✓ GPS location verification');
    if (analysis.hasPhotos)
        evidence.push(`✓ ${analysis.photosCount} timestamped photo(s)`);
    if (analysis.hasChatHistory)
        evidence.push('✓ Chat history reviewed');
    if (analysis.hasWaitTimeDocumentation)
        evidence.push('✓ Wait time documented');
    if (analysis.descriptionWordCount > 50)
        evidence.push('✓ Detailed description provided');
    if (evidence.length === 0)
        return 'Limited evidence available';
    return evidence.join('\n');
}
function getDecisionReasoning(analysis, decision, isDoer) {
...
// ============================================
export function generateCancellationPenaltyMessageToDoer(doerName, jobTitle, penalty, hoursBeforeStart) {
    const penaltyDescription = hoursBeforeStart > 4
...
export function generateCancellationRefundMessageToAsker(askerName, jobTitle, refundAmount, creditAmount) {
    return `
...
// APPEAL MESSAGES
// ============================================
export function generateAppealReceivedMessage(userName, decision) {
    return `
...
export function generateAppealDecisionMessage(userName, originalDecision, newDecision, reasoning) {
    const outcomeText = newDecision === 'appeal_upheld'
