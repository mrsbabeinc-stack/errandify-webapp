 ...
 */
export async function storePushSubscription(userId, subscription) {
    try {
        const existingSubscription = await db.query('SELECT id FROM push_subscriptions WHERE user_id = $1 AND endpoint = $2', [userId, subscription.endpoint]);
        if (existingSubscription.rows.length > 0) {
            // Update existing subscription
            await db.query(`UPDATE push_subscriptions
         SET auth_key = $1, p256dh_key = $2, user_agent = $3, updated_at = NOW()
         WHERE id = $4`, [subscription.keys.auth, subscription.keys.p256dh, subscription.userAgent, existingSubscription.rows[0].id]);
            console.log('Updated push subscription for user:', userId);
        }
        else {
            // Create new subscription
            await db.query(`INSERT INTO push_subscriptions (user_id, endpoint, auth_key, p256dh_key, user_agent)
         VALUES ($1, $2, $3, $4, $5)`, [userId, subscription.endpoint, subscription.keys.auth, subscription.keys.p256dh, subscription.userAgent]);
...
export async function getUserPushSubscriptions(userId) {
    try {
        const result = await db.query(`SELECT id, endpoint, auth_key, p256dh_key, created_at
    ...
    try {
        ...
        for (const subscription of subscriptions) {
            try {
                ...
                // Log failed send
                await logPushNotification(subscription.id, userId, payload, 'failed', error.message || 'Unknown error');
            }
        }
 ...
 */
export async function broadcastPushNotification(userIds, payload) {
    let totalSent = 0;
    let totalFailed = 0;
 ...
 */
async function logPushNotification(subscriptionId, userId, payload, status, errorMessage) {
    try {
        await db.query(`INSERT INTO push_notification_logs
