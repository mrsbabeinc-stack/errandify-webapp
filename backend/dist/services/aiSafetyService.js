...
export async function assessUserVulnerability(userId) {
    try {
        const userResult = await db.query(`SELECT id, display_name, created_at, average_rating, total_jobs_completed
       FROM users WHERE id = $1`, [userId]);
        if (userResult.rows.length === 0) {
            return { vulnerabilityScore: 0, riskFactors: [], recommendedSupport: [] };
        }
        const user = userResult.rows[0];
        const capResult = await db.query(`SELECT physical_limitations, health_conditions, vulnerable_support_needs
       FROM user_capabilities WHERE user_id = $1`, [userId]);
...
Return JSON: {"score": number, "riskFactors": [], "supportNeeded": []}`;
        ...
        let assessment = { score: 0.3, riskFactors: [], supportNeeded: [] };
        try {
...
export async function detectCoercion(userId, timeframe = 'month') {
    try {
        const interval = timeframe === 'day' ? '1 day' : timeframe === 'week' ? '7 days' : '30 days';
        const activityResult = await db.query(`SELECT COUNT(*) as jobs_accepted, AVG(e.budget) as avg_budget
       FROM errands e WHERE doer_id = $1 AND completed_at > NOW() - INTERVAL $2`, [userId, interval]);
...
async function createAdminAlert(params) {
    try {
        await db.query(`INSERT INTO safety_flags (user_id, flag_type, severity, description, markers)
       VALUES ($1, $2, $3, $4, $5)`, [params.userId, params.type, params.severity, `AI-detected ${params.type}`, JSON.stringify(params.evidence)]);
    }
    catch (error) {
