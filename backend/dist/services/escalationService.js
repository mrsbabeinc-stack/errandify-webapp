 ...
 */
export async function escalateDisputeToL2(disputeId, aiConfidence, aiRecommendation, aiReasoning) {
    try {
        // Create escalation record
        await db.query(`INSERT INTO dispute_escalations
       (dispute_id, level, ai_confidence, ai_recommendation, ai_reasoning, status)
       VALUES ($1, $2, $3, $4, $5, 'open')`, [disputeId, 2, aiConfidence, aiRecommendation, aiReasoning]);
        // Update dispute status
        await db.query('UPDATE disputes SET status = $1 WHERE id = $2', ['escalated_to_l2', disputeId]);
 ...
 */
export async function assignDisputeToAgent(escalationId, agentId) {
    try {
        await db.query(`UPDATE dispute_escalations
       SET assigned_to_user_id = $1, status = 'in_progress'
       WHERE id = $2`, [agentId, escalationId]);
 ...
 */
export async function getL2DisputesForAgent(agentId) {
    try {
       ...
       ORDER BY e.created_at DESC`, [agentId]);
        return result.rows;
    }
    catch (error) {
 ...
 */
export async function resolveL2Dispute(escalationId, decision, reasoning, agentId) {
    try {
       ...
       WHERE id = $4`, [decision, reasoning, agentId, escalationId]);
        // Get dispute ID
        const result = await db.query('SELECT dispute_id FROM dispute_escalations WHERE id = $1', [escalationId]);
        const disputeId = result.rows[0].dispute_id;
        // Update dispute status based on decision
        await db.query('UPDATE disputes SET status = $1 WHERE id = $2', [`resolved_l2_${decision}`, disputeId]);
 ...
 */
export async function createAppeal(disputeId, appealReason, newEvidenceUrl, userId) {
    try {
        const result = await db.query(`INSERT INTO dispute_appeals
       (dispute_id, appeal_reason, new_evidence_url, appealed_by_user_id, status)
       VALUES ($1, $2, $3, $4, 'pending')
       RETURNING id`, [disputeId, appealReason, newEvidenceUrl, userId]);
        const appealId = result.rows[0].id;
        // Update dispute status
        await db.query('UPDATE disputes SET status = $1 WHERE id = $2', ['escalated_to_l3', disputeId]);
 ...
 */
export async function getL3Appeals() {
    try {
        const result = await db.query(`SELECT 
        a.id as appeal_id,
        a.dispute_id,
 ...
 */
export async function resolveAppeal(appealId, decision, reasoning, agentId) {
    try {
        // Update appeal with final decision
        await db.query(`UPDATE dispute_appeals
       SET l3_decision = $1, l3_reasoning = $2, decided_by_user_id = $3, decided_at = NOW(), status = 'final'
       WHERE id = $4`, [decision, reasoning, agentId, appealId]);
        // Get dispute ID
        const result = await db.query('SELECT dispute_id FROM dispute_appeals WHERE id = $1', [appealId]);
        const disputeId = result.rows[0].dispute_id;
        // Update dispute status
        await db.query('UPDATE disputes SET status = $1 WHERE id = $2', [`resolved_l3_${decision}`, disputeId]);
 ...
 */
async function addToQueue(disputeId, priority, category) {
    try {
        await db.query(`INSERT INTO support_queue (dispute_id, priority, category, status)
       VALUES ($1, $2, $3, 'open')`, [disputeId, priority, category]);
    }
    catch (error) {
