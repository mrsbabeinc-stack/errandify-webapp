 ...
 */
export async function calculateAdPayment(companyId, campaignId, campaignTitle, budgetCents) {
 ...
 */
export async function processAdPayment(companyId, campaignId, campaignTitle, budgetCents, stripeCustomerId, stripeCardToken) {
    try {
        ...
        if (calculation.credits_to_use_cents > 0) {
            ...
            await db.query(`INSERT INTO ad_credit_usage_log (company_id, campaign_id, amount, action, created_at)
         VALUES ($1, $2, $3, 'campaign_approved_credit_deducted', NOW())`, [companyId, campaignId, calculation.credits_to_use_cents]);
        }
        ...
        if (calculation.requires_stripe_payment && calculation.stripe_amount_cents > 0) {
            ...
            try {
                ...
                // Log Stripe charge
                await db.query(`INSERT INTO ad_credit_usage_log (company_id, campaign_id, amount, action, created_at)
           VALUES ($1, $2, $3, 'campaign_approved_stripe_charged', NOW())`, [companyId, campaignId, calculation.stripe_amount_cents]);
            }
            catch (stripeError) {
       ...
       SET status = 'approved',
           approved_at = NOW(),
           stripe_charge_id = $1
       WHERE id = $2`, [stripeChargeId || null, campaignId]);
 ...
 */
export async function createStripeSession(companyId, campaignId, campaignTitle, stripeAmountCents, returnUrl) {
    try {
 ...
 */
export async function getCompanyAdCreditStatus(companyId) {
    try {
        const credits = await adCreditService.getCredits(companyId);
        // Get subscription from database directly
        const subResult = await db.query('SELECT subscription_tier FROM company_subscriptions WHERE company_id = $1 LIMIT 1', [companyId]);
