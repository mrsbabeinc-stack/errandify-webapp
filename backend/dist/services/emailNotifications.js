...
import { templateBidAccepted, templateTaskReopened, templatePaymentReleased, templateDailyDigest, templatePaymentReminder, templateReferralJoin, templateFirstJobBonus, templateRatingReceived, } from '../templates/emailTemplates.js';
// Send email for critical events (immediate)
export async function sendCriticalEmail(userId, emailType, data) {
    try {
        // Get user email
        const userResult = await db.query('SELECT email, display_name FROM users WHERE id = $1', [userId]);
        ...
        const prefsResult = await db.query('SELECT email_frequency, email_preferences FROM users WHERE id = $1', [userId]);
        ...
        switch (emailType) {
            case 'bid_accepted':
                ...
                html = templateBidAccepted(user.display_name, data.taskTitle, data.amount, data.taskId);
                break;
            case 'task_reopened':
                subject = `🎯 Task Available Again! Your bid of $${data.bidAmount}`;
                html = templateTaskReopened(user.display_name, data.taskTitle, data.bidAmount, data.taskId);
                break;
...
export async function queueForDigest(userId, notificationId) {
    try {
        await db.query(`INSERT INTO email_digest_queue (user_id, notification_id, queued_at)
       VALUES ($1, $2, NOW())
...
export async function sendDailyDigests() {
    try {
        ...
        const usersResult = await db.query(`SELECT DISTINCT user_id FROM email_digest_queue
       WHERE sent_at IS NULL`, []);
        for (const row of usersResult.rows) {
            const userId = row.user_id;
            // Get all queued notifications for this user
            const queuedResult = await db.query(`SELECT n.id, n.title, n.body, n.type
         ...
         LIMIT 20`, [userId]);
            ...
            const summary = {
                bids: queuedResult.rows
                    .filter((n) => n.type === 'new_bid_received')
                    .map((n) => ({
            ...
            // Mark as sent
            if (success) {
                await db.query(`UPDATE email_digest_queue
           SET sent_at = NOW()
...
export async function sendPaymentReminders() {
    try {
        ...
        const result = await db.query(`SELECT e.id, e.asker_id, e.title, e.created_at, u.display_name, u.email
       FROM errands e
       ...
       AND NOT EXISTS (
        ...
        for (const errand of result.rows) {
            ...
            // Log email sent
            if (success) {
                await db.query(`INSERT INTO email_logs (user_id, email_type, subject, sent_at, errand_id)
           VALUES ($1, 'payment_reminder', $2, NOW(), $3)`, [errand.asker_id, `Payment reminder for ${errand.title}`, errand.id]);
...
function extractTaskTitle(body) {
