...
export async function moderateContent(content, context = 'task_description') {
    try {
...
Return JSON: {"toxicity": 0.0-1.0, "isScam": true/false, "isCoercion": true/false, "isSexual": true/false, "reason": ""}`,
            ...
            let aiResult = { toxicity: 0, isScam: false, isCoercion: false, isSexual: false };
            try {
...
export async function flagContent(params) {
    try {
        await db.query(`INSERT INTO flagged_content
...
export async function getFlaggedContent(limit = 50, severity, status) {
    try {
...
export async function reviewFlaggedContent(contentId, decision, notes) {
    try {
        const result = await db.query(`UPDATE flagged_content
       SET status = $1, reviewed_by_user_id = $2, reviewed_at = NOW(), review_notes = $3
       WHERE id = $4
       RETURNING *`, [decision, 0, notes || null, contentId]);
 ...
 */
export async function moderatePhotoContent(photoUrl, context = 'job_completion') {
    try {
