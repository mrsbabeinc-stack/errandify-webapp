 ...
 */
export async function awardEp(request) {
    ...
    try {
       ...
       VALUES ($1, 0, 0, 'bronze')
       ON CONFLICT (user_id) DO NOTHING`, [request.userId]);
        // Reset monthly EP if we've crossed into a new month
        const userResult = await client.query(`SELECT current_month_ep, created_at FROM user_gamification WHERE user_id = $1`, [request.userId]);
       ...
       RETURNING total_ep, current_month_ep`, [awardAmount, request.userId]);
        ...
        await client.query(`UPDATE user_gamification SET tier = $1 WHERE user_id = $2`, [newTier, request.userId]);
...
export async function getUserGamification(userId) {
    const result = await db.query(`SELECT total_ep, current_month_ep, tier, login_streak
     FROM user_gamification
     WHERE user_id = $1`, [userId]);
    if (result.rows.length === 0) {
        // Create if doesn't exist
        await db.query(`INSERT INTO user_gamification (user_id, total_ep, current_month_ep, tier)
       VALUES ($1, 0, 0, 'bronze')`, [userId]);
        return {
            totalEp: 0,
            currentMonthEp: 0,
            tier: 'bronze',
 ...
 */
export async function getEpTransactions(userId, limit = 50) {
    const result = await db.query(`SELECT id, amount, reason, related_errand_id, created_at
 ...
 */
function calculateTier(totalEp) {
    if (totalEp >= TIER_THRESHOLDS.platinum)
        return 'platinum';
    if (totalEp >= TIER_THRESHOLDS.gold)
        return 'gold';
    if (totalEp >= TIER_THRESHOLDS.silver)
        return 'silver';
    return 'bronze';
}
/**
 ...
 */
function getNextTier(currentTier, totalEp) {
    ...
    const nextTierThreshold = TIER_THRESHOLDS[nextTierName];
 ...
 * Get rating bonus EP
 */
export function getRatingBonus(rating) {
    return RATING_BONUSES[rating] || 0;
}
