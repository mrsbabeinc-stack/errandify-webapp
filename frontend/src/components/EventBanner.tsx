...
export default function EventBanner() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-2">
      <div className={`${styles.bg} ${styles.border} rounded-lg p-3 flex items-center justify-between gap-3 transition-all duration-300 min-h-[60px]`}>
        {/* LEFT: Icon + Message */}
        <div className="flex items-center gap-2.5 flex-1 min-w-0">
          <span className="text-lg flex-shrink-0">{currentEvent.icon}</span>
          <p className={`${styles.text} text-sm font-semibold`}>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          {currentEvent.action && (
            <a
              ...
              className={`${styles.action} text-sm font-bold px-3 py-1 hover:underline whitespace-nowrap`}
            >
          ...
          {SAMPLE_EVENTS.length > 1 && (
            <button
              onClick={handleNext}
              className={`${styles.action} text-sm font-bold px-2 py-1 hover:bg-white hover:bg-opacity-50 rounded transition`}
              title="Next event"
            >
              ›
            </button>
