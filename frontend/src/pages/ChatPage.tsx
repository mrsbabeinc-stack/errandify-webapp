            ...
            <div key={conversation.id} style={{minWidth: 0, background: 'linear-gradient(135deg, #FFFBF8 0%, #FFF6F0 100%)', borderRadius: '12px', padding: '10px 12px', boxShadow: '0 2px 12px rgba(255, 107, 53, 0.12)', border: '1px solid #FFE0CC', position: 'relative', transition: 'all 0.2s cubic-bezier(0.34, 1.56, 0.64, 1)', cursor: 'pointer'}} onMouseEnter={(e) => {e.currentTarget.style.boxShadow = '0 6px 20px rgba(255, 107, 53, 0.18)'; e.currentTarget.style.transform = 'translateY(-2px)';}} onMouseLeave={(e) => {e.currentTarget.style.boxShadow = '0 2px 12px rgba(255, 107, 53, 0.12)'; e.currentTarget.style.transform = 'translateY(0)';}}>
              ...
              <div style={{display: 'flex', alignItems: 'center', gap: '12px', minWidth: 0, marginBottom: '5px', fontSize: '12px', color: '#888', fontWeight: '500'}}>
                ...
                {(conversation.location || conversation.postal) && (
                  <span style={{whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', minWidth: 0}}>📍 {conversation.postal}{conversation.location && conversation.postal ? ', ' : ''}{conversation.location}</span>
                )}
              </div>

              {/* Line 3: Status + By ... Chat */}
              <div style={{display: 'flex', alignItems: 'center', gap: '8px', justifyContent: 'space-between'}}>
                <div style={{display: 'flex', alignItems: 'center', gap: '8px', minWidth: 0, flex: 1}}>
                  <span style={{padding: '4px 10px', borderRadius: '8px', fontSize: '12px', fontWeight: '700', whiteSpace: 'nowrap', flexShrink: 0, background: getStatusColor(conversation.status).includes('orange') ? 'linear-gradient(135deg, #FFD89B 0%, #FFC870 100%)' : getStatusColor(conversation.status).includes('green') ? 'linear-gradient(135deg, #A8E6C1 0%, #7DD9B5 100%)' : 'linear-gradient(135deg, #E8E8E8 0%, #D8D8D8 100%)', color: getStatusColor(conversation.status).includes('orange') ? '#B85A1D' : getStatusColor(conversation.status).includes('green') ? '#2E7D32' : '#555'}}>
